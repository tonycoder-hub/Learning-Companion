...
- Adds app-focused `File > Export Workspace...` (`Shift+Cmd+E`) and `File > Import Workspace...` (`Cmd+O`) commands for local JSON backup/restore and append-only phone/Windows return-file import without browser downloads.
- Routes text-based web Export-panel Save actions through the same native NSSavePanel bridge, so Workspace/Review Pack/Markdown/JSON/Today/Mirror saves do not fall back to browser downloads inside the Mac shell.
