...
export function generateReviewHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "        <button id=\"copyProgressBtn\" class=\"secondary\" type=\"button\">Copy Return JSON</button>",
    "        <button id=\"downloadProgressBtn\" type=\"button\">Save Return JSON</button>",
    ...
    "      link.href = url;",
    "      link.download = returnFileName('learning-companion-review-progress-patch', patch.patchId);",
    "      link.click();",
    "      URL.revokeObjectURL(url);",
    "      markSaved();",
    "      setStatus('Return JSON saved. Move it back to the Mac and import it from Today > Return Files.');",
    "    });",
...
export function generateInboxHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      <div class=\"actions\">",
    "        <button id=\"copyPatchBtn\" class=\"secondary\" type=\"button\">Copy Return JSON</button>",
    "        <button id=\"downloadPatchBtn\" type=\"button\">Save Return JSON</button>",
    ...
    "      const link = document.createElement('a');",
    "      link.href = url;",
    "      link.download = returnFileName('learning-companion-inbox-patch', patch.patchId);",
    "      link.click();",
    "      URL.revokeObjectURL(url);",
    "      markSaved();",
    "      setStatus('Return JSON saved. Move it back to the Mac and import it from Today > Return Files.');",
...
export function generateMirrorIndexHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "    <section class=\"panel\">",
    "      <h2>Manual Return</h2>",
    "      <p>This is a static mirror, not live sync. On phone or Windows, use Review or Inbox, save a return JSON, move that file back to the Mac, then import it from Today &gt; Return Files.</p>",
    "    </section>",
    "    <section class=\"panel\">",
    "      <h2>Resume Here</h2>",
