...
export function generateInboxHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      try { const patch = buildPatch(); await navigator.clipboard.writeText(JSON.stringify(patch, null, 2)); markSaved(); setStatus(`Return file copied. Name it ${returnFileName('learning-companion-inbox-patch', patch.patchId)} when you save it.`); }",
    ...
    "      saveDrafts();",
    "      clearForm();",
    "      setStatus('Capture added to return draft. Save the return file when ready.');",
    "      render();",
    "    }",
    "    function buildPatch() {",
