...
export function generateReviewHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      <span>Grade cards here, then save <code>learning-companion-review-progress-patch.json</code>. Move that JSON back to the Mac and import it from Today &gt; Return Files.</span>",
    "    </section>",
    ...
    "      try { await navigator.clipboard.writeText(JSON.stringify(buildPatch(), null, 2)); setStatus('Return JSON copied.'); }",
...
export function generateInboxHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      <h2>Return to Mac</h2>",
    "      <p class=\"summary\">Add captures here, then save <code>learning-companion-inbox-patch.json</code>. Move that JSON back to the Mac and import it from Today &gt; Return Files.</p>",
    "    </section>",
