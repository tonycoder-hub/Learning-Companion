...
export function generateReviewHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      document.querySelector('#returnFileHint').textContent = `Suggested JSON file: ${returnFileName('learning-companion-review-progress-patch', patch.patchId)}`;",
...
export function generateInboxHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "    document.querySelector('#clearDraftsBtn').addEventListener('click', () => { drafts = []; returnMeta = resetReturnMeta('inbox_patch'); answerContextState = ''; answerContextQuestion = ''; answerToCaptureId = ''; saveDrafts(); markSaved(); render(); });",
    ...
    "      catch { selectReturnJson('patchPreview', 'Copy failed. Return file selected; copy it manually.'); }",
    "    });",
    ...
    "        setStatus('Answer captured in return draft. Save the return file when ready.');",
    "      } else setStatus('Capture added to return draft. Save the return file when ready.');",
    "      render();",
    "    }",
    "    function buildPatch() {",
    ...
    "      document.querySelector('#returnFileHint').textContent = `Suggested JSON file: ${returnFileName('learning-companion-inbox-patch', patch.patchId)}`;",
