...
export function generateReviewHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "        <button id=\"copyProgressBtn\" type=\"button\">Copy Return JSON</button>",
    "        <button id=\"downloadProgressBtn\" class=\"secondary\" type=\"button\">Save Return JSON</button>",
    "        <button id=\"selectProgressBtn\" class=\"secondary\" type=\"button\" aria-label=\"Select Return JSON for manual copy\">Manual Copy</button>",
    "        <button id=\"clearProgressBtn\" class=\"secondary\" type=\"button\">Clear Progress</button>",
    "      </div>",
    "      <output id=\"progressStatus\">Grade cards here, then save a return JSON for the Mac app.</output>",
    ...
    "      try { const patch = buildPatch(); await navigator.clipboard.writeText(JSON.stringify(patch, null, 2)); markSaved(); setStatus(`Return JSON copied. Name it ${returnFileName('learning-companion-review-progress-patch', patch.patchId)} when you save it.`); }",
    "      catch { selectReturnJson('progressPreview', 'Copy failed. Return JSON selected; copy it manually.'); }",
    "    });",
    "    document.querySelector('#selectProgressBtn').addEventListener('click', () => { selectReturnJson('progressPreview', 'Return JSON selected. Copy it manually, then bring it back to the Mac.'); });",
    ...
    "      if (!mode) return;",
    "      markSaved();",
    "      setStatus(mode === 'picker' ? 'Return JSON saved. Move it back to the Mac and import it from Today > Return Files.' : 'Return JSON download requested. Move it back to the Mac and import it from Today > Return Files.');",
    "    });",
    ...
    "      document.querySelector('#returnFileHint').textContent = `Suggested file: ${returnFileName('learning-companion-review-progress-patch', patch.patchId)}`;",
    ...
    "      setStatus(count ? `${count} review ${count === 1 ? 'event' : 'events'} ready. Save Return JSON, move it back to the Mac, then import from Today > Return Files.` : 'Grade cards here, then save a return JSON for the Mac app.');",
    "    }",
    ...
    "          return 'picker';",
    "        } catch (error) {",
    "          if (error?.name !== 'AbortError') setStatus('Save failed. Use Copy Return JSON instead.');",
    "          return '';",
    "        }",
    "      }",
    "      if (!shouldUseFallbackDownload()) { setStatus('Save picker unavailable here. Use Copy Return JSON instead.'); return ''; }",
...
export function generateInboxHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      </div>",
    "      <output id=\"statusOutput\">Drafts stay in this browser until you save a return JSON for the Mac app.</output>",
    "    </section>",
    "    <section class=\"panel\" aria-label=\"Draft captures\">",
    "      <h2>Draft Captures</h2>",
    "      <div id=\"draftList\"></div>",
    "      <div class=\"actions\">",
    "        <button id=\"copyPatchBtn\" type=\"button\">Copy Return JSON</button>",
    "        <button id=\"downloadPatchBtn\" class=\"secondary\" type=\"button\">Save Return JSON</button>",
    "        <button id=\"selectPatchBtn\" class=\"secondary\" type=\"button\" aria-label=\"Select Return JSON for manual copy\">Manual Copy</button>",
    ...
    "    document.querySelector('#clearDraftsBtn').addEventListener('click', () => { drafts = []; returnMeta = resetReturnMeta('inbox_patch'); saveDrafts(); markSaved(); render(); });",
    "    document.querySelector('#copyPatchBtn').addEventListener('click', async () => {",
    "      try { const patch = buildPatch(); await navigator.clipboard.writeText(JSON.stringify(patch, null, 2)); markSaved(); setStatus(`Return JSON copied. Name it ${returnFileName('learning-companion-inbox-patch', patch.patchId)} when you save it.`); }",
    "      catch { selectReturnJson('patchPreview', 'Copy failed. Return JSON selected; copy it manually.'); }",
    "    });",
    "    document.querySelector('#selectPatchBtn').addEventListener('click', () => { selectReturnJson('patchPreview', 'Return JSON selected. Copy it manually, then bring it back to the Mac.'); });",
    ...
    "      saveDrafts();",
    "      clearForm();",
    "      setStatus('Capture added to patch draft. Save Return JSON when ready.');",
    "      render();",
    "    }",
    "    function buildPatch() {",
    ...
    "      document.querySelector('#returnFileHint').textContent = `Suggested file: ${returnFileName('learning-companion-inbox-patch', patch.patchId)}`;",
    ...
    "    function clearForm() { Object.values(fields).forEach((field) => { field.value = ''; }); sourceUrlExplicit = false; answerToCaptureId = ''; }",
...
export function generateMirrorIndexHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "        <li><strong>Return JSON back to Mac</strong><span>Save a return JSON, move that file back yourself, then import it from Today &gt; Return Files.</span></li>",
    "      </ol>",
