...
export function generateReviewHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "    function storageUnavailableStatus(prefix = 'Review progress staged in this return file.') { return `${prefix} Browser storage is unavailable, so keep this page open and use Manual Copy or Save before closing.`; }",
    ...
    "        }",
    "      }",
    "      if (!shouldUseFallbackDownload()) { selectReturnJson('progressPreview', 'Save picker unavailable here. Return file selected; copy it manually.'); return ''; }",
    "      try {",
