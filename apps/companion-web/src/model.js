...
function staticRuntimeI18nScriptLines() {
  return [
    ...
    "    function i18nRuntimeHtml(en, zh) { return `<span class=\"i18n-en\">${escapeRuntimeHtml(en)}</span><span class=\"i18n-zh\" lang=\"zh-CN\">${escapeRuntimeHtml(zh)}</span>`; }",
    "    function setI18nHtml(node, en, zh) { if (node) node.innerHTML = i18nRuntimeHtml(en, zh); }",
    "    function i18nRuntimeText(en, zh) { return document.querySelector('#mirrorLangZh')?.checked ? zh : en; }"
  ];
}
...
export function generateReviewHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      event.returnValue = '';",
    "    });",
    "    document.querySelectorAll('input[name=\"mirrorLanguage\"]').forEach((input) => { input.addEventListener('change', renderSaveMode); });",
    "    function buildPatch() {",
...
export function generateInboxHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    `      <label><span id="quoteLabel">${i18nText("Quote", "引文")}</span><textarea id="quoteInput" placeholder="Paste a quote or transcript line / 粘贴引文或转写行"></textarea></label>`,
    `      <label><span id="thoughtLabel">${i18nText("Thought", "想法")}</span><textarea id="thoughtInput" placeholder="Your thought, question, or takeaway / 你的想法、问题或收获"></textarea></label>`,
    "      <div class=\"row\">",
    ...
    "      </div>",
    "      <div class=\"row\">",
    `        <label>${i18nText("Source", "来源")}<input id="sourceTitleInput" placeholder="Page or video title / 页面或视频标题"></label>`,
    ...
    "    document.querySelectorAll('input[name=\"mirrorLanguage\"]').forEach((input) => { input.addEventListener('change', () => { renderSaveMode(); renderAnswerContext(); }); });",
    ...
    "      fields.quote.placeholder = isAnswerDraft ? i18nRuntimeText('Question carried from the Mac mirror', '从 Mac 镜像带来的问题') : i18nRuntimeText('Paste a quote or transcript line', '粘贴引文或转写行');",
    "      fields.thought.placeholder = isAnswerDraft ? i18nRuntimeText('Write the answer to bring back to Mac', '写下要带回 Mac 的回答') : i18nRuntimeText('Your thought, question, or takeaway', '你的想法、问题或收获');",
