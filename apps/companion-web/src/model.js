...
export function generateReviewHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      <span>Grade cards here, then save a timestamped review return file. Move that file back to the Mac and import it from Today &gt; Return Files.</span>",
    "    </section>",
    "    <section class=\"progress-panel\" aria-label=\"Review progress patch\">",
    "      <div class=\"progress-actions\">",
    "        <button id=\"copyProgressBtn\" type=\"button\">Copy Return File</button>",
    "        <button id=\"downloadProgressBtn\" class=\"secondary\" type=\"button\">Save Return File</button>",
    "        <button id=\"selectProgressBtn\" class=\"secondary\" type=\"button\" aria-label=\"Select return file for manual copy\">Manual Copy</button>",
    "        <button id=\"clearProgressBtn\" class=\"secondary\" type=\"button\">Clear Progress</button>",
    "      </div>",
    "      <output id=\"progressStatus\">Grade cards here, then save a return file for the Mac app.</output>",
    ...
    "    });",
    "    document.querySelector('#selectProgressBtn').addEventListener('click', () => { if (!hasReturnProgress()) { setStatus(noReviewReturnMessage()); return; } selectReturnJson('progressPreview', 'Return file selected. Copy it manually, then bring it back to the Mac.'); });",
    ...
    "      markSaved();",
    "      renderProgress();",
    "      if (!savedProgress) setStatus(storageUnavailableStatus('Review progress cleared in memory.'));",
    "    }",
    ...
    "      document.querySelector('#returnNextStep').textContent = count ? `${count} review ${count === 1 ? 'event' : 'events'} staged in this return file. Use Copy or ${returnFileActionVerb()} to take it back to Mac before closing.` : noReviewReturnMessage();",
    "      if (!count || hasUnsavedProgress()) hideReturnAfterSave();",
    "      setStatus(count ? `${count} review ${count === 1 ? 'event' : 'events'} ready. Save the return file, move it back to the Mac, then import from Today > Return Files.` : noReviewReturnMessage());",
    "    }",
    ...
    "    function noReviewReturnMessage() { return seed.cards.length ? 'No review return file yet. Reveal and grade a card first.' : 'No due cards in this mirror. Nothing to return from Review; close this tab or use Inbox for notes.'; }",
    ...
    "        preview.setAttribute('tabindex', '-1');",
    "        preview.focus();",
    "        setStatus(`${message} If selection did not work, manually highlight the return file below.`);",
    "        return false;",
    "      }",
    "    }",
    ...
    "          return 'picker';",
    "        } catch (error) {",
    "          if (error?.name !== 'AbortError') setStatus('Save failed. Use Copy Return File instead.');",
    "          return '';",
    "        }",
    "      }",
    "      if (!shouldUseFallbackDownload()) { renderProgress(); selectReturnJson('progressPreview', 'Save picker unavailable here. Return file selected for manual copy. Nothing was saved to disk.'); return ''; }",
    "      try {",
    ...
    "        return 'download';",
    "      } catch {",
    "        selectReturnJson('progressPreview', 'Save failed. Return file selected; copy it manually.');",
    "        return '';",
    "      }",
    "    }",
    ...
    "    function returnFileActionVerb() { if (canUseSavePicker()) return 'Save'; if (shouldUseFallbackDownload()) return 'Download'; return 'Manual Copy'; }",
    ...
    "      if (canUseSavePicker()) { button.textContent = 'Save Return File'; mode.textContent = 'This browser can save with a file picker. Choose where to keep the return file.'; return; }",
    "      if (shouldUseFallbackDownload()) { button.textContent = 'Download Return File'; mode.textContent = 'Automated download fallback is enabled for this controlled test run. Ordinary browsers should use Copy or a file picker.'; return; }",
    "      button.textContent = 'Select Return File';",
    "      mode.textContent = 'No file picker detected. Use Copy Return File, or select the preview and save it manually in Files or a text editor.';",
    "    }",
    ...
    "    renderSaveMode();",
    "    renderProgress();",
    "    if (!storageAvailable) setStatus(storageUnavailableStatus('Review progress is memory-only.'));",
    "  </script>",
    "</body>",
    "</html>",
    ""
...
export function generateInboxHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      <h2>Return to Mac</h2>",
    "      <p class=\"summary\">Add captures here, then save a timestamped inbox return file. Move that file back to the Mac and import it from Today &gt; Return Files.</p>",
    "    </section>",
    ...
    "        <strong id=\"answerContextTitle\">You're answering a question from this mirror</strong>",
    "        <q id=\"answerQuestionPreview\"></q>",
    "        <span id=\"answerContextText\">Your answer will be saved to a return file you move back to Mac.</span>",
    "      </div>",
    "      <label>Topic<select id=\"topicSelect\" aria-describedby=\"topicSourceHint\"></select></label>",
    "      <p id=\"topicSourceHint\" class=\"meta\"></p>",
    "      <label><span id=\"quoteLabel\">Quote</span><textarea id=\"quoteInput\" placeholder=\"Paste a quote or transcript line\"></textarea></label>",
    "      <label><span id=\"thoughtLabel\">Thought</span><textarea id=\"thoughtInput\" placeholder=\"Your thought, question, or takeaway\"></textarea></label>",
    "      <div class=\"row\">",
    ...
    "      </div>",
    "      <output id=\"statusOutput\">Drafts stay in this browser until you save a return file for the Mac app.</output>",
    "    </section>",
    "    <section class=\"panel\" aria-label=\"Draft captures\">",
    "      <h2>Draft Captures</h2>",
    "      <div id=\"draftList\"></div>",
    "      <div class=\"actions\">",
    "        <button id=\"copyPatchBtn\" type=\"button\">Copy Return File</button>",
    "        <button id=\"downloadPatchBtn\" class=\"secondary\" type=\"button\">Save Return File</button>",
    "        <button id=\"selectPatchBtn\" class=\"secondary\" type=\"button\" aria-label=\"Select return file for manual copy\">Manual Copy</button>",
    ...
    "    });",
    "    document.querySelector('#selectPatchBtn').addEventListener('click', () => { if (!hasCurrentTopicDrafts()) { setStatus(noInboxReturnMessage()); return; } selectReturnJson('patchPreview', 'Return file selected. Copy it manually, then bring it back to the Mac.'); });",
    ...
    "        setStatus(savedDrafts ? 'Answer captured in return draft. Save the return file when ready.' : storageUnavailableStatus('Answer captured in this return draft.'));",
    "      } else setStatus(savedDrafts ? 'Capture added to return draft. Save the return file when ready.' : storageUnavailableStatus('Capture added to this return draft.'));",
    "      render();",
    "    }",
    "    function clearDrafts() {",
    "      if (!drafts.length) { setStatus('No draft captures to clear.'); return; }",
    "      drafts = [];",
    ...
    "      markSaved();",
    "      render();",
    "      if (!savedDrafts) setStatus(storageUnavailableStatus('Drafts cleared in memory.'));",
    "    }",
    "    function buildPatch() {",
    ...
    "      document.querySelector('#returnFileHint').textContent = `Suggested JSON file: ${filename}`;",
    "      document.querySelector('#returnManualHelp').textContent = `Locked-down browser: use Manual Copy, press Ctrl+C or Command+C, or long-press the selected text on phone, paste into a text editor such as Notepad, and save as ${filename} before moving it back to Mac.`;",
    "      document.querySelector('#returnNextStep').textContent = topicDrafts.length ? `${topicDrafts.length} draft ${topicDrafts.length === 1 ? 'capture' : 'captures'} staged in this return file. Use Copy or ${returnFileActionVerb()} to take it back to Mac before closing.` : noInboxReturnMessage();",
    ...
    "      if (answerToCaptureId || fields.quote.value || fields.thought.value) setStatus(notice || 'Answer draft loaded from mirror link.');",
    "      renderAnswerContext();",
    "    }",
    ...
    "      answerContextTitle.textContent = answerContextState === 'captured' ? 'Answer captured in this return draft' : \"You're answering a question from this mirror\";",
    "      answerQuestionPreview.textContent = question;",
    "      answerContextText.textContent = answerContextState === 'captured' ? 'Save or copy the return file to move it back to Mac.' : 'Your answer will be saved to a return file you move back to Mac.';",
    "    }",
    "    function setAnswerFieldMode(isAnswerDraft) {",
    "      fieldLabels.quote.textContent = isAnswerDraft ? 'Question from Mac' : 'Quote';",
    "      fieldLabels.thought.textContent = isAnswerDraft ? 'Answer to return' : 'Thought';",
    "      fields.quote.placeholder = isAnswerDraft ? 'Question carried from the Mac mirror' : 'Paste a quote or transcript line';",
    "      fields.thought.placeholder = isAnswerDraft ? 'Write the answer to bring back to Mac' : 'Your thought, question, or takeaway';",
    ...
    "    function noInboxReturnMessage() { return 'No draft captures for this topic yet. Add a quote or thought before saving a return file.'; }",
    ...
    "    function storageUnavailableStatus(prefix = 'Draft staged in this return file.') { return `${prefix} Browser storage is unavailable, so keep this page open and use Copy, Manual Copy, or the available save action before closing.`; }",
    ...
    "      if (!hint) return;",
    "      if (fields.sourceTitle.value.trim() || fields.sourceUrl.value.trim()) { hint.textContent = 'Using the Source or URL you entered for this capture.'; return; }",
    "      hint.textContent = topic.sourceTitle || topic.sourceUrl ? `Source: ${topic.sourceTitle || 'Linked source'} - used for new captures unless you fill Source or URL below.` : 'This mirror has no topic source. Add Source or URL if this capture needs one.';",
    "    }",
    ...
    "        }",
    "      }",
    "      if (!shouldUseFallbackDownload()) { render(); selectReturnJson('patchPreview', 'Save picker unavailable here. Return file selected for manual copy. Nothing was saved to disk.'); return ''; }",
    "      try {",
    "        const url = URL.createObjectURL(blob);",
    "        const link = document.createElement('a');",
    "        link.href = url;",
    ...
    "        return 'download';",
    "      } catch {",
    "        selectReturnJson('patchPreview', 'Save failed. Return file selected; copy it manually.');",
    "        return '';",
    "      }",
    "    }",
    "    function canUseSavePicker() { return typeof window.showSaveFilePicker === 'function' && !allowsAutomatedDownloadFallback(); }",
    ...
    "    renderSaveMode();",
    "    render();",
    "    if (!storageAvailable) setStatus(storageUnavailableStatus('Drafts are memory-only.'));",
    "  </script>",
    "</body>",
    "</html>",
    ""
