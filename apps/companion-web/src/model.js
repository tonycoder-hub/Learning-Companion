...
export function generateReviewHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      try { await navigator.clipboard.writeText(JSON.stringify(buildPatch(), null, 2)); markSaved(); setStatus('Return JSON copied.'); }",
    ...
    "    });",
    "    document.querySelector('#clearProgressBtn').addEventListener('click', () => { progress = { events: {} }; saveProgress(); markSaved(); renderProgress(); });",
    ...
    "    function returnFileName(prefix, id) { return `${prefix}-${compactLocalStamp(new Date())}-${shortId(id)}.json`; }",
    ...
    "        }",
    "      }",
    "      if (!shouldUseFallbackDownload()) { setStatus('Save picker unavailable in this automated browser. Use Copy Return JSON instead.'); return ''; }",
    ...
    "      return 'download';",
    "    }",
    "    function canUseSavePicker() { return typeof window.showSaveFilePicker === 'function' && navigator.webdriver !== true && !allowsAutomatedDownloadFallback(); }",
    "    function shouldUseFallbackDownload() { return navigator.webdriver !== true || allowsAutomatedDownloadFallback(); }",
...
export function generateInboxHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "    document.querySelector('#clearDraftsBtn').addEventListener('click', () => { drafts = []; saveDrafts(); markSaved(); render(); });",
