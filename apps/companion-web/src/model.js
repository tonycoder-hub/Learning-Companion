...
export function generateReviewHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      catch { selectReturnJson('progressPreview', 'Copy failed. Return file selected; copy it manually.'); }",
    "    });",
    ...
    "    });",
    "    document.querySelector('#clearProgressBtn').addEventListener('click', () => { progress = { events: {} }; returnMeta = resetReturnMeta('review_patch'); saveProgress(); markSaved(); renderProgress(); });",
    ...
    "      document.querySelector('#returnManualHelp').textContent = `Locked-down browser: use Manual Copy, press Ctrl+C, paste into a text editor such as Notepad, and save as ${filename} before moving it back to Mac.`;",
    "      document.querySelector('#returnNextStep').textContent = count ? `${count} review ${count === 1 ? 'event' : 'events'} staged in this return file. Use Copy or Save to take it back to Mac before closing.` : 'No review events yet. Mark a due card to start a return file for Mac.';",
    ...
    "    function resetReturnMeta(prefix) { const next = { patchId: makeId(prefix), stamp: compactLocalStamp(new Date()) }; localStorage.setItem(returnMetaKey, JSON.stringify(next)); return next; }",
    ...
    "    function selectReturnJson(previewId, message) { const preview = document.querySelector(`#${previewId}`); if (!preview) return; const range = document.createRange(); range.selectNodeContents(preview); const selection = window.getSelection(); selection.removeAllRanges(); selection.addRange(range); preview.setAttribute('tabindex', '-1'); preview.focus(); setStatus(message); }",
    ...
    "        }",
    "      }",
    "      if (!shouldUseFallbackDownload()) { setStatus('Save picker unavailable here. Use Copy Return File instead.'); return ''; }",
...
export function generateInboxHtml(workspace, now = new Date()) {
  ...
  return [
    ...
    "      document.querySelector('#returnFileHint').textContent = `Suggested JSON file: ${filename}`;",
    "      document.querySelector('#returnManualHelp').textContent = `Locked-down browser: use Manual Copy, press Ctrl+C, paste into a text editor such as Notepad, and save as ${filename} before moving it back to Mac.`;",
    "      document.querySelector('#returnNextStep').textContent = topicDrafts.length ? `${topicDrafts.length} draft ${topicDrafts.length === 1 ? 'capture' : 'captures'} staged in this return file. Use Copy or Save to take it back to Mac before closing.` : 'No draft captures yet. Add a quote or thought to start a return file for Mac.';",
    "      renderAnswerContext();",
    "    }",
    ...
    "    function storageUnavailableStatus(prefix = 'Draft staged in this return file.') { return `${prefix} Browser storage is unavailable, so keep this page open and use Manual Copy or Save before closing.`; }",
    ...
    "        }",
    "      }",
    "      if (!shouldUseFallbackDownload()) { selectReturnJson('patchPreview', 'Save picker unavailable here. Return file selected; copy it manually.'); return ''; }",
    "      try {",
    "        const url = URL.createObjectURL(blob);",
    "        const link = document.createElement('a');",
    "        link.href = url;",
