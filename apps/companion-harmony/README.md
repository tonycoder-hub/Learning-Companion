...
- `readerNextAction` contains `kind`, `label`, `detail`, `route`, `routeLabel`, `meta`, `secondary`, optional `secondaryAction`, `generatedAt`, and `surface: "reader"`. Routes are limited to `pages/ReviewQueue`, `pages/TopicDetail`, or `pages/ImportReceipt` until real navigation is compiled. `secondaryAction` is only emitted when the primary phone action hides another useful lane, such as due review plus open questions; its optional `routeParams.section` distinguishes `open_questions` and `answers_today` inside the existing TopicDetail route.
...
- Patch envelopes include both `source.workspaceFingerprint` and `source.returnBaseFingerprint`, so future Harmony return files can use the Mac return-base safety check instead of falling back to the legacy full-workspace check. If `source.returnBaseFingerprint` exists, it is the only merge-base decision input; `source.workspaceFingerprint` remains freshness/legacy evidence.
