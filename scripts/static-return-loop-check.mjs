...
function assertStaticReturnContract(html, {
  ...
  assert.match(html, /Move it to Mac, then import or paste it from Today &gt; Return Files|Move it to Mac, then import or paste it from Today > Return Files/);
  assert.match(html, /If a file was saved: Windows - check Downloads/);
  assert.match(html, /HarmonyOS phone - check the Files app&#39;s Downloads folder|HarmonyOS phone - check the Files app's Downloads folder/);
  assert.match(html, /other browsers - check the browser's default download folder/);
  assert.match(html, /If no file was created: use Copy or Manual Copy/);
  assert.match(html, /paste the return JSON into a trusted note, email, or message/);
  assert.match(html, /Files app&#39;s Downloads folder|Files app's Downloads folder/);
  assert.match(html, /browser's default download folder/);
  assert.match(html, /Manual carriers after you have the JSON: AirDrop, USB, file share, email/);
  assert.match(html, /Feishu Drive; no live sync/);
