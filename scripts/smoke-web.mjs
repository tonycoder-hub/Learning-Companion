...
const readerJs = readFileSync("apps/companion-web/src/reader.js", "utf8");
...
const platformQaEvidenceScaffoldJs = readFileSync("scripts/platform-qa-evidence-scaffold.mjs", "utf8");
const platformQaHandoffBindingJs = readFileSync("scripts/lib/platform-qa-handoff-binding.mjs", "utf8");
const platformQaEvidenceFilesJs = readFileSync("scripts/lib/platform-qa-evidence-files.mjs", "utf8");
const isoDateTimeHelperJs = readFileSync("scripts/lib/iso-date-time.mjs", "utf8");
...
assert.equal(packageJson.scripts["external:approval-check"], "node scripts/external-source-validation-browser.mjs --approval-check");
...
assert.equal(packageJson.scripts["mac:manual:validate:real"], "node scripts/validate-mac-manual-qa.mjs --qa dist/morning-demo/MAC_MANUAL_QA.md --platform-handoff .codex-tmp/platform-qa-handoff/current.json --out .codex-tmp/mac-manual-qa/real-run-receipt.json --require-claimable");
...
assert.equal(packageJson.scripts["windows:static:validate:real"], "node scripts/validate-windows-static-qa.mjs --qa dist/morning-demo/WINDOWS_STATIC_QA.md --platform-handoff .codex-tmp/platform-qa-handoff/current.json --out .codex-tmp/windows-static-qa/real-run-receipt.json --require-claimable");
...
assert.match(sourceApprovalFreshnessHelperJs, /approvalCheck/);
assert.match(sourceApprovalFreshnessHelperJs, /external:approval-check/);
assert.match(sourceApprovalFreshnessHelperJs, /source-approval-check\.json/);
...
assert.match(platformQaHandoffSelfTestJs, /custom\/mac real\.json/);
assert.match(platformQaHandoffSelfTestJs, /custom\/windows real\.json/);
assert.match(platformQaHandoffSelfTestJs, /custom\/harmony real\.json/);
assert.match(platformQaHandoffSelfTestJs, /nextCommands\.finalizeNextMajor/);
assert.match(platformQaHandoffSelfTestJs, /nextCommands\.finalKoGateWithExplicitPlatformReceipts/);
assert.match(platformQaHandoffSelfTestJs, /--mac-manual requires a Mac manual QA receipt path/);
...
assert.equal(packageJson.scripts["platform:qa-evidence-scaffold"], "node scripts/platform-qa-evidence-scaffold.mjs");
assert.equal(packageJson.scripts["platform:qa-evidence-scaffold:selftest"], "node scripts/platform-qa-evidence-scaffold.mjs --self-test");
assert.match(packageJson.scripts.smoke, /platform-qa-evidence-scaffold\.mjs --self-test/);
...
assert.match(platformQaValidatorsSelfTestJs, /assertPlatformQaEvidenceFileBinding/);
assert.match(platformQaValidatorsSelfTestJs, /assertFilledRowsRequirePlatformHandoff/);
assert.match(platformQaValidatorsSelfTestJs, /must reference row-specific evidence notes/);
assert.match(platformQaValidatorsSelfTestJs, /must reference row-specific evidence screenshot/);
assert.match(platformQaValidatorsSelfTestJs, /evidence screenshot missing/);
assert.match(platformQaValidatorsSelfTestJs, /evidence screenshot file must not be empty/);
...
assert.match(platformQaValidatorsSelfTestJs, /2026-02-31T08:00:00\+08:00/);
...
assert.match(platformQaEvidenceFilesJs, /platformQaEvidenceFileErrors/);
assert.match(platformQaEvidenceFilesJs, /rowEvidenceHints/);
assert.match(platformQaEvidenceFilesJs, /must reference row-specific evidence notes/);
assert.match(platformQaEvidenceFilesJs, /must reference row-specific evidence screenshot/);
assert.match(platformQaEvidenceFilesJs, /evidence notes file missing/);
assert.match(platformQaEvidenceFilesJs, /evidence screenshot missing/);
assert.match(platformQaEvidenceFilesJs, /evidence screenshot file must not be empty/);
assert.match(platformQaEvidenceFilesJs, /evidence screenshot file must be a PNG/);
...
assert.match(platformQaEvidenceFilesJs, /from "\.\/iso-date-time\.mjs"/);
...
assert.match(isoDateTimeHelperJs, /export const ISO_DATE_TIME_PATTERN/);
assert.match(isoDateTimeHelperJs, /export function isIsoDateTimeWithTimezone/);
assert.match(isoDateTimeHelperJs, /daysInMonth/);
assert.match(isoDateTimeHelperJs, /month >= 1/);
assert.match(isoDateTimeHelperJs, /day <= daysInMonth/);
assert.match(isoDateTimeHelperJs, /offsetHour <= 23/);
...
assert.match(nextMajorReadinessSelfTestJs, /sourceApprovalCheck/);
...
assert.match(nextMajorOperatorSelfTestJs, /check-current-turn-source-approval/);
assert.match(nextMajorOperatorSelfTestJs, /external:approval-check/);
...
assert.match(indexHtml, /id="shortcutOverlay" class="shortcut-overlay" role="dialog" aria-modal="true" aria-labelledby="shortcutOverlayTitle" hidden/);
assert.match(indexHtml, /id="shortcutOverlayTitle">Shortcuts<\/h2>/);
assert.match(indexHtml, /id="shortcutCloseBtn" class="icon-button shortcut-close" type="button" aria-label="Close shortcuts"/);
...
assert.match(appJs, /function renderShortcutOverlayCopy\(\)/);
assert.match(appJs, /Open this shortcuts sheet", "打开这个快捷键表"/);
assert.match(appJs, /event\.key === "\?"/);
assert.match(appJs, /event\.key === "Escape"[\s\S]+closeShortcutOverlay\(\)/);
...
assert.match(readerJs, /Can't load the content\? Paste the article below\./);
assert.match(readerJs, /无法加载内容？请把文章粘贴到下方。/);
assert.match(readerJs, /Paste article content\.\.\./);
assert.match(readerJs, /粘贴文章内容\.\.\./);
assert.doesNotMatch(readerJs, /Can't load\? Paste content below/);
assert.doesNotMatch(readerJs, /粘贴文章内容\.\.\. \/ Paste article content\.\.\./);
assert.match(smokeBrowserJs, /fallbackEn/);
assert.match(smokeBrowserJs, /fallbackZh/);
...
assert.match(externalSourcePrivacyReviewJs, /source-approval-check\.v1/);
assert.match(externalSourcePrivacyReviewJs, /SOURCE_APPROVAL_CHECK_ONLY/);
assert.match(externalSourcePrivacyReviewJs, /pre-check only and cannot be privacy-reviewed into KO evidence/);
...
assert.match(externalSourcePrivacyReviewJs, /source approval check receipt rejected/);
assert.match(externalSourcePrivacyReviewJs, /source approval check template rejected/);
...
assert.match(externalSourcePrivacyReviewJs, /screenshot privacy review must be PASS/);
...
assert.match(externalSourcePrivacyReviewJs, /duplicate screenshot review entry rejected/);
assert.match(externalSourcePrivacyReviewJs, /extra screenshot review file rejected/);
assert.match(externalSourcePrivacyReviewJs, /failed screenshot review status rejected/);
...
assert.match(externalSourcePrivacyReviewJs, /invalid calendar reviewedAt timestamp rejected/);
assert.match(externalSourcePrivacyReviewJs, /from "\.\/lib\/iso-date-time\.mjs"/);
...
assert.match(koEvidenceReviewJs, /external-source-privacy-review\.v1/);
assert.match(koEvidenceReviewJs, /function assertExternalPrivacyReviewArtifact/);
assert.match(koEvidenceReviewJs, /function assertExternalPrivacyReviewScreenshots/);
assert.match(koEvidenceReviewJs, /external claim review receipt path must match claim/);
assert.match(koEvidenceReviewJs, /external claim review verdict must be PASS/);
assert.match(koEvidenceReviewJs, /external claim review screenshot sha256 must match claim/);
...
assert.match(koEvidenceReviewJs, /pre-check only and cannot be used as privacy-reviewed KO evidence/);
...
assert.match(koEvidenceReviewJs, /invalid calendar external reviewedAt timestamp rejected/);
assert.match(koEvidenceReviewJs, /failed external privacy review artifact rejected/);
assert.match(koEvidenceReviewJs, /mismatched external privacy review receipt path rejected/);
assert.match(koEvidenceReviewJs, /mismatched external privacy review screenshot rejected/);
...
assert.match(koEvidenceReviewJs, /platformFailureStatus/);
assert.match(koEvidenceReviewJs, /platformFailureDetail/);
...
assert.match(koEvidenceReviewJs, /source approval check external evidence rejected/);
assert.match(koEvidenceReviewJs, /pending platform evidence rejected/);
assert.match(koEvidenceReviewJs, /pendingMacRequirement\?\.status, "PENDING_NOT_RUN"/);
assert.match(koEvidenceReviewJs, /missingNotesMacRequirement\?\.status, "INVALID_OR_INCOMPLETE"/);
assert.match(koEvidenceReviewJs, /partialMacRequirement\?\.status, "PARTIAL_OR_BLOCKED_RUN"/);
...
assert.match(koEvidenceReviewJs, /platformQaEvidenceFileErrors/);
...
assert.match(koEvidenceReviewJs, /platform PASS rows without row-specific evidence screenshot rejected/);
...
assert.match(koEvidenceReviewJs, /screenshot: \${screenshotPath}/);
assert.match(koEvidenceReviewJs, /PNG_1X1/);
...
assert.match(koEvidenceReviewJs, /platform invalid calendar Date\/time rejected/);
assert.match(koEvidenceReviewJs, /from "\.\/lib\/iso-date-time\.mjs"/);
...
assert.match(koEvidenceReviewJs, /assertNoPlatformEvidenceErrors/);
...
assert.match(koEvidenceReviewJs, /function platformHandoffArtifactErrors/);
assert.match(koEvidenceReviewJs, /platformHandoffBinding handoff artifact missing/);
assert.match(koEvidenceReviewJs, /platformHandoffBinding receiptPath is missing/);
assert.match(koEvidenceReviewJs, /platformHandoffBinding receiptPath must match actual receipt path/);
assert.match(koEvidenceReviewJs, /platformHandoffBinding handoff artifact platform receiptPath is missing/);
assert.match(koEvidenceReviewJs, /platformHandoffBinding handoff artifact receiptPath must match binding/);
...
assert.match(koEvidenceReviewJs, /missing platform handoff artifact rejected/);
assert.match(koEvidenceReviewJs, /copied platform receipt with stale binding path rejected/);
assert.match(koEvidenceReviewJs, /missing platform handoff receipt path rejected/);
...
assert.match(koEvidenceReviewJs, /mismatched platform handoff artifact receipt path rejected/);
...
assert.match(finalizeNextMajorJs, /localEvidence: "\.codex-tmp\/next-major-local-evidence\/current\.json"/);
...
assert.match(finalizeNextMajorJs, /id: "check-local-evidence-snapshot"/);
assert.match(finalizeNextMajorJs, /scripts\/refresh-next-major-local-evidence\.mjs/);
assert.match(finalizeNextMajorJs, /"--check"/);
assert.match(finalizeNextMajorJs, /"--local-evidence-out",\s*options\.localEvidence/);
...
assert.match(finalizeNextMajorJs, /operator packet readiness freshness must be current/);
assert.match(finalizeNextMajorJs, /operator packet platform handoff freshness must be current/);
assert.match(finalizeNextMajorJs, /Local evidence snapshot checked:/);
assert.match(finalizeNextMajorJs, /operator final lane readinessReady must be true/);
assert.match(finalizeNextMajorJs, /operator final lane platformHandoffReady must be true/);
assert.match(finalizeNextMajorJs, /operator final lane must be ready to validate final KO/);
...
assert.match(finalizeNextMajorJs, /const externalClaim = await readJson\(plan\.options\.external\)/);
assert.match(finalizeNextMajorJs, /const sourceApprovalRequest = await readJson\(plan\.options\.sourceApprovalRequest\)/);
assert.match(finalizeNextMajorJs, /if \(command\.output\) await ensureParentDirectory\(command\.output\)/);
...
assert.match(finalizeNextMajorJs, /function assertFinalKoOutputBindings/);
assert.match(finalizeNextMajorJs, /assertFinalizerOutputBindings\(plan, \{ ko, externalClaim, sourceApprovalRequest, readiness, operator \}\)/);
assert.match(finalizeNextMajorJs, /async function runFinalizePlan\(plan\)[\s\S]*assertFinalizerOutputBindings\(plan, \{ ko, externalClaim, sourceApprovalRequest, readiness, operator \}\)/);
assert.match(finalizeNextMajorJs, /external claim schema/);
assert.match(finalizeNextMajorJs, /external claim source approval request path must match finalizer input/);
assert.match(finalizeNextMajorJs, /external claim approved reading URL must match source approval request/);
assert.match(finalizeNextMajorJs, /external claim source approval basis git HEAD must match source approval request/);
assert.match(finalizeNextMajorJs, /final KO source approval request path must match finalizer input/);
assert.match(finalizeNextMajorJs, /final KO source approval request path must match external claim/);
assert.match(finalizeNextMajorJs, /final KO requested approval text must match source approval request/);
assert.match(finalizeNextMajorJs, /final KO source approval basis receipt path must match source approval request/);
assert.match(finalizeNextMajorJs, /function assertResolvedPathEqual/);
assert.match(finalizeNextMajorJs, /actual path must be present/);
...
assert.match(finalizeNextMajorJs, /id: "refresh-platform-handoff"[\s\S]*"--mac-manual",\s*options\.macManual/);
assert.match(finalizeNextMajorJs, /id: "refresh-platform-handoff"[\s\S]*"--windows-static",\s*options\.windowsStatic/);
assert.match(finalizeNextMajorJs, /id: "refresh-platform-handoff"[\s\S]*"--harmony-device",\s*options\.harmonyDevice/);
...
assert.match(refreshNextMajorLocalEvidenceJs, /assertLocalEvidenceOutputsFresh/);
assert.match(refreshNextMajorLocalEvidenceJs, /public dry-run receipt git HEAD must match current HEAD/);
assert.match(refreshNextMajorLocalEvidenceJs, /source approval request basis receipt/);
assert.match(refreshNextMajorLocalEvidenceJs, /path must match local evidence snapshot/);
assert.match(refreshNextMajorLocalEvidenceJs, /KO next source approval freshness/);
...
assert.match(refreshNextMajorLocalEvidenceJs, /doesNotProve=/);
...
assert.match(refreshNextMajorLocalEvidenceJs, /refresh-static-return-contract/);
assert.match(refreshNextMajorLocalEvidenceJs, /refresh-return-import-dry-run/);
assert.match(refreshNextMajorLocalEvidenceJs, /refresh-dogfood-runbook-pending/);
assert.match(refreshNextMajorLocalEvidenceJs, /refresh-mac-manual-pending/);
assert.match(refreshNextMajorLocalEvidenceJs, /refresh-windows-static-pending/);
assert.match(refreshNextMajorLocalEvidenceJs, /refresh-harmony-device-pending/);
...
assert.match(refreshNextMajorLocalEvidenceJs, /PLATFORM_QA_EVIDENCE_SCAFFOLD_SCHEMA/);
assert.match(refreshNextMajorLocalEvidenceJs, /platformEvidenceScaffoldOut/);
assert.match(refreshNextMajorLocalEvidenceJs, /platform-qa-evidence-scaffold\.mjs/);
assert.match(refreshNextMajorLocalEvidenceJs, /platformQaEvidenceScaffold/);
assert.match(refreshNextMajorLocalEvidenceJs, /must not create screenshots/);
assert.match(refreshNextMajorLocalEvidenceJs, /scripts\/static-return-loop-check\.mjs/);
assert.match(refreshNextMajorLocalEvidenceJs, /scripts\/return-file-import-dry-run\.mjs/);
assert.match(refreshNextMajorLocalEvidenceJs, /scripts\/validate-dogfood-runbook\.mjs/);
assert.match(refreshNextMajorLocalEvidenceJs, /scripts\/validate-mac-manual-qa\.mjs/);
assert.match(refreshNextMajorLocalEvidenceJs, /scripts\/validate-windows-static-qa\.mjs/);
assert.match(refreshNextMajorLocalEvidenceJs, /scripts\/validate-harmony-device-qa\.mjs/);
...
[macManualQaValidatorJs, windowsStaticQaValidatorJs, harmonyDeviceQaValidatorJs].forEach((validatorJs) => {
  ...
  assert.match(validatorJs, /from "\.\/lib\/iso-date-time\.mjs"/);
  ...
  assert.match(validatorJs, /platformQaEvidenceFileErrors/);
  assert.match(validatorJs, /from "\.\/lib\/platform-qa-areas\.mjs"/);
  assert.match(validatorJs, /from "\.\/lib\/platform-qa-evidence-files\.mjs"/);
  ...
  assert.match(validatorJs, /--require-claimable/);
  assert.match(validatorJs, /requireClaimable/);
  assert.match(validatorJs, /needs a full all-PASS real run/);
...
assert.match(koNextActionSummaryJs, /Approval pre-check command/);
...
assert.match(koNextActionSummaryJs, /npm run external:approval-check/);
...
assert.match(nextMajorReadinessJs, /sourceApprovalCheck: buildSourceApprovalCheckCommand/);
assert.match(nextMajorReadinessJs, /function buildSourceApprovalCheckCommand/);
assert.match(nextMajorReadinessJs, /source-approval-check\.json/);
assert.match(nextMajorReadinessJs, /parts\.push\("--external", shellQuote\(externalEvidencePath\)\)/);
assert.match(nextMajorReadinessJs, /platformHandoff: buildPlatformHandoffCommand/);
assert.match(nextMajorReadinessJs, /function buildPlatformHandoffCommand/);
...
assert.match(nextMajorOperatorPacketJs, /check-current-turn-source-approval/);
assert.match(nextMajorOperatorPacketJs, /function buildApprovalCheckCommandFromRequest/);
...
assert.match(nextMajorOperatorPacketJs, /Approval checks do not run browser evidence/);
...
assert.match(nextMajorOperatorPacketJs, /scriptInThisDir\("platform-qa-handoff\.mjs"\)[\s\S]*\.\.\.buildCustomPlatformReceiptArgv\(platformReceiptPaths\)/);
...
try {
  ...
  assert.equal(operatorPacket.nextActionSequence.some((step) => step.id === "check-current-turn-source-approval"), true);
  ...
  assert.match(operatorMarkdown, /check-current-turn-source-approval/);
  ...
  assert.match(koNextConsole, /check-current-turn-source-approval/);
  ...
  assert.match(missingSourceKoNextConsole, /Approval pre-check command: .*external:approval-check/);
...
assert.match(platformQaHandoffJs, /--mac-manual requires a Mac manual QA receipt path/);
assert.match(platformQaHandoffJs, /--windows-static requires a Windows static\/manual QA receipt path/);
assert.match(platformQaHandoffJs, /--harmony-device requires a HarmonyOS device QA receipt path/);
assert.match(platformQaHandoffJs, /function platformConfigsFromArgs/);
assert.match(platformQaHandoffJs, /function buildCustomPlatformReceiptCommandArgs/);
...
assert.match(platformQaHandoffJs, /from "\.\/lib\/iso-date-time\.mjs"/);
assert.match(platformQaHandoffJs, /function shellQuote/);
assert.match(platformQaHandoffJs, /"mac-manual"/);
assert.match(platformQaHandoffJs, /"windows-static"/);
assert.match(platformQaHandoffJs, /"harmony-device"/);
...
assert.match(platformQaEvidenceScaffoldJs, /learning-companion\.platform-qa-evidence-scaffold\.v1/);
assert.match(platformQaEvidenceScaffoldJs, /PLATFORM_QA_EVIDENCE_SCAFFOLD_ONLY/);
assert.match(platformQaEvidenceScaffoldJs, /canClaimKo: false/);
assert.match(platformQaEvidenceScaffoldJs, /canClaimPlatformQa: false/);
assert.match(platformQaEvidenceScaffoldJs, /screenshot\.png\.TODO\.txt/);
assert.match(platformQaEvidenceScaffoldJs, /screenshotCreated: false/);
assert.match(platformQaEvidenceScaffoldJs, /TEMPLATE ONLY - replace before use/);
assert.match(platformQaEvidenceScaffoldJs, /platform_qa_evidence_scaffold_selftest_ok/);
assert.match(platformQaEvidenceScaffoldJs, /writePrivateFileIfAbsent/);
assert.match(platformQaEvidenceScaffoldJs, /must stay inside/);
assert.match(platformQaEvidenceScaffoldJs, /must use forward slashes/);
assert.match(platformQaEvidenceScaffoldJs, /assertPathUnderSuggestedEvidenceRoot/);
...
assert.match(externalSourceValidationBrowserJs, /function runSourceApprovalCheck/);
...
assert.match(externalSourceValidationBrowserJs, /source_approval_check_ok/);
assert.match(externalSourceValidationBrowserJs, /source_approval_check_error/);
...
assert.match(externalSourceValidationBrowserJs, /source-approval-check\.v1/);
...
assert.match(externalSourceValidationBrowserJs, /SOURCE_APPROVAL_CHECK_ONLY/);
...
assert.match(externalSourceValidationBrowserJs, /--approval-check requires --source-approval-request/);
assert.match(externalSourceValidationBrowserJs, /--approval-check requires --approval-note/);
...
assert.match(externalSourceValidationBrowserJs, /approvalNoteMatched: true/);
...
assert.match(externalSourceValidationBrowserJs, /No source approval was granted by this check artifact/);
assert.match(externalSourceValidationBrowserJs, /Approval Pre-Check After Current-Turn Approval/);
assert.match(externalSourceValidationBrowserJs, /Browser Candidate After Pre-Check/);
...
assert.match(inboxHtml, /placeholder="Paste a quote or transcript line"/);
assert.match(inboxHtml, /placeholder="Your thought, question, or takeaway"/);
assert.match(inboxHtml, /placeholder="Page or video title"/);
assert.match(inboxHtml, /setI18nPlaceholder\(fields\.quote/);
assert.match(inboxHtml, /setI18nPlaceholder\(fields\.thought/);
assert.match(inboxHtml, /setI18nPlaceholder\(fields\.sourceTitle/);
assert.doesNotMatch(inboxHtml, /Paste a quote or transcript line \/ 粘贴引文或转写行/);
assert.doesNotMatch(inboxHtml, /Your thought, question, or takeaway \/ 你的想法、问题或收获/);
assert.doesNotMatch(inboxHtml, /Page or video title \/ 页面或视频标题/);
assert.match(smokeBrowserJs, /zhPlaceholders/);
assert.match(smokeBrowserJs, /enAfterSwitchPlaceholders/);
