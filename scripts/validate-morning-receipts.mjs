...
assert.match(manualQa, /npm run mac:manual:validate:real/);
assert.doesNotMatch(manualQa, /npm run mac:manual:validate(?!:(?:smoke|real))/);
...
assert.match(manualQa, /_中文：学习伴侣 Mac 手动 QA 回执_/);
assert.match(manualQa, /中文阶段：内部构建手动 QA/);
assert.match(manualQa, /_中文：会话信息_/);
assert.match(manualQa, /_中文：前置条件_/);
assert.match(manualQa, /_中文：测试矩阵_/);
assert.match(manualQa, /中文填写说明：在晨间 review 时填写/);
assert.match(manualQa, /真实 Mac GUI 运行/);
assert.match(manualQa, /For every non-`NT` row, fill Notes with the concrete evidence reference/);
assert.match(manualQa, /每个非 `NT` 行都必须在 Notes 写入具体证据引用/);
assert.match(manualQa, /Every `PASS`, `FAIL`, or `BLOCKED` row must include a concrete Notes evidence reference/);
assert.match(manualQa, /## 中文行指引/);
assert.match(manualQa, /源时间暂存/);
assert.match(manualQa, /问题闭环/);
assert.match(manualQa, /镜像检查/);
assert.match(windowsStaticQa, /^# Learning Companion Windows Static QA Receipt$/m);
assert.match(windowsStaticQa, /_中文：学习伴侣 Windows 静态 QA 回执_/);
assert.match(windowsStaticQa, /EVIDENCE: PENDING_USER_GATE/);
assert.match(windowsStaticQa, /PENDING RECEIPT, not QA evidence/);
assert.match(windowsStaticQa, /中文阶段：便携 fixture 手动 QA/);
assert.match(windowsStaticQa, /_中文：会话信息_/);
assert.match(windowsStaticQa, /_中文：前置条件_/);
assert.match(windowsStaticQa, /_中文：测试矩阵_/);
assert.match(windowsStaticQa, /中文填写说明：在 Windows Edge\/Chrome 静态镜像运行时使用/);
assert.match(windowsStaticQa, /For every non-`NT` row, fill Notes with the concrete evidence reference/);
assert.match(windowsStaticQa, /每个非 `NT` 行都必须在 Notes 写入具体证据引用/);
assert.match(windowsStaticQa, /Every `PASS`, `FAIL`, or `BLOCKED` row must include a concrete Notes evidence reference/);
assert.match(windowsStaticQa, /## 中文行指引/);
assert.match(windowsStaticQa, /预返回指纹检查/);
assert.match(windowsStaticQa, /批量部分导入保护/);
assert.match(windowsStaticQa, /静态边界/);
...
assert.match(windowsStaticQa, /npm run windows:static:validate:real/);
assert.doesNotMatch(windowsStaticQa, /npm run windows:static:validate(?!:(?:smoke|real))/);
assert.match(windowsStaticQa, /both must be `PASS` before this receipt can support a Windows static-loop usability claim/);
assert.match(windowsStaticQa, /这个回执才能支撑 Windows 静态闭环可用性结论/);
...
assert.match(harmonyDeviceQa, /_中文：学习伴侣 HarmonyOS 设备 QA 回执_/);
assert.match(harmonyDeviceQa, /EVIDENCE: PENDING_USER_GATE/);
assert.match(harmonyDeviceQa, /PENDING RECEIPT, not device evidence/);
assert.match(harmonyDeviceQa, /中文阶段：设备手动 QA/);
assert.match(harmonyDeviceQa, /_中文：会话信息_/);
assert.match(harmonyDeviceQa, /_中文：前置条件_/);
assert.match(harmonyDeviceQa, /_中文：测试矩阵_/);
assert.match(harmonyDeviceQa, /中文填写说明：在 HarmonyOS DevEco\/工具链和设备\/模拟器运行时使用/);
assert.match(harmonyDeviceQa, /For every non-`NT` row, fill Notes with the concrete evidence reference/);
assert.match(harmonyDeviceQa, /每个非 `NT` 行都必须在 Notes 写入具体证据引用/);
assert.match(harmonyDeviceQa, /Every `PASS`, `FAIL`, or `BLOCKED` row must include a concrete Notes evidence reference/);
assert.match(harmonyDeviceQa, /## 中文行指引/);
assert.match(harmonyDeviceQa, /文件候选保护/);
assert.match(harmonyDeviceQa, /失败导入保留/);
assert.match(harmonyDeviceQa, /Review patch 导出/);
...
assert.match(harmonyDeviceQa, /npm run harmony:device:validate:real/);
assert.doesNotMatch(harmonyDeviceQa, /npm run harmony:device:validate(?!:(?:smoke|real))/);
...
assert.match(harmonyDeviceQa, /这个回执才能支撑 HarmonyOS 设备往返可用性结论/);
