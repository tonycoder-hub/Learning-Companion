...
const EXPECTED_QA_ROWS = MAC_MANUAL_QA_AREAS.length;
...
function printHelp() {
  console.log("Usage: node scripts/validate-mac-manual-qa.mjs [--qa dist/morning-demo/MAC_MANUAL_QA.md] [--platform-handoff .codex-tmp/platform-qa-handoff/current.json] [--out receipt.json] [--require-claimable]");
...
function validateMacManualQa(markdown, qaPath, currentRevision, { platformHandoffBinding = null, platformHandoffErrors = [] } = {}) {
  ...
  rows.forEach((row, index) => {
    ...
    const expectedArea = MAC_MANUAL_QA_AREAS[index] || "";
...
async function main() {
  ...
  const modeErrors = [];
  if (options.requireClaimable && receipt.claimBoundary.canClaimMacManualQaUsable !== true) {
    modeErrors.push("Mac manual QA --require-claimable needs a full all-PASS real run with passing native build and browser smoke gates.");
  }
