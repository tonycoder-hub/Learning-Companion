...
function buildMacManualQaMarkdown({
  ...
  return [
    ...
    "| Source time staging | Paste a supported video URL that contains a timestamp into the source URL field. | The Time field is filled and pulsed, the activity strip reports `Source time staged`, the source-open button reports that time, and the stored source URL no longer includes only the time parameter. | NT |  |",
