...
function buildMacManualQaMarkdown({
  ...
  return [
    ...
    "- Validate a filled receipt with `npm run mac:manual:validate:real` before making a Mac manual-QA usability claim.",
    "- 中文：在声明 Mac 手动 QA 可用前，先用 `npm run mac:manual:validate:real` 校验已填写回执。",
...
function buildWindowsStaticQaMarkdown({
  ...
  return [
    ...
    "- Validate a filled receipt with `npm run windows:static:validate:real` before making a Windows static-loop usability claim.",
    "- 中文：在声明 Windows 静态闭环可用前，先用 `npm run windows:static:validate:real` 校验已填写回执。",
...
function buildHarmonyDeviceQaMarkdown() {
  return [
    ...
    "- Validate a filled receipt with `npm run harmony:device:validate:real` before making a HarmonyOS device-roundtrip usability claim.",
    "- 中文：在声明 HarmonyOS 设备往返可用前，先用 `npm run harmony:device:validate:real` 校验已填写回执。",
