...
function buildMacManualQaMarkdown({
  ...
  return [
    ...
    "| Source time staging | Paste a supported video URL that contains a timestamp into the source URL field, then use `-15` and `+15`. | The Time field is filled and pulsed, Quick Capture shows the source/time context, the context Open button includes that time, the nudge buttons adjust the context time, the activity strip reports `Source time staged` or `Time adjusted`, and the stored source URL no longer includes only the time parameter. | NT |  |",
