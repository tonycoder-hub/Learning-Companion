...
function buildDogfoodRunbookMarkdown({
  ...
  return [
    ...
    "| 5 | Insert one capture into Notes, view the generated block, then decide whether to use the source-return hint. | Notes preview opens the actual block without hiding user-authored text; source-return friction is counted instead of guessed. | NT |  |",
    ...
    "- For Add-to-Notes, count how often you immediately prefer the source-return hint over `View note`; if source-return wins 3 or more times, record it as Mac loop friction and demote `View note` in the next implementation pass.",
