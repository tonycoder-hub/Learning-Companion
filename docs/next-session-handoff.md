...
- Static Review/Inbox return hints now explain where a browser-saved return JSON may be found without implying the app writes to Downloads by default. The shared hint says that if the browser saved a file, check Downloads on Windows, Files > Downloads on a Harmony phone, or the browser's default download folder on other devices; if no file was created, use Copy or Manual Copy and paste into a trusted note app, with Feishu explicitly labeled as a manual carrier only. README now also states that static Review/Inbox files must be moved back to Mac manually and that live Feishu sync is not verified. Mira returned `PASS_WITH_NOTES`; accepted follow-up made the location language conditional, added the other-device fallback, de-emphasized Feishu as manual-only, and loosened static/browser assertions away from one brittle sentence. Deferred: real Windows/Harmony visual verification and real manual device loop.
