...
- Quick Capture now shows a visible draft outcome summary inside the capture context, so the learner sees what the current quote/thought will become, where it will land, and whether source resume is ready before saving. The summary distinguishes Open Questions, Answers Today, linked-question closure, highlights, takeaways, generic captures, missing source, timestamped source resume, and source-changed drafts. Browser smoke pins the summary in full desk and sidecar readability checks, source/timestamp staging, question/answer intent changes, no-source state, and source re-anchor warnings. This is controlled web evidence only; it does not prove Mac Accessibility/Automation selected-text capture or real dogfood comprehension.
- The offline morning headline gate now runs the HarmonyOS device QA validator too. `scripts/morning-check.sh` runs `npm run harmony:device:validate:smoke` after the dogfood, Mac manual, and Windows static validators, and `validate-morning-receipts.mjs` asserts the package aliases, shell gate entries, order, generated dashboard/script wording, Stage Matrix `harmony_device_qa` row, and generated receipt content stay aligned. `HARMONY_DEVICE_QA.md` now has a real-run Session Header (`Date/time`, reviewer, HarmonyOS device/build, app build/source, DevEco/toolchain result, import/return transfer/Mac import methods, Mac Return Files result, elapsed time, file-picker/storage friction, patch export/import friction, biggest friction). `scripts/validate-harmony-device-qa.mjs` writes `learning-companion.harmony-device-qa-receipt.v1`, accepts the all-`NT` pending receipt with `canClaimHarmonyDeviceRoundtripUsable=false`, rejects missing FAIL/BLOCKED notes, rejects fully executed rows with required header fields still `TBD`, and requires both DevEco/toolchain and Mac Return Files import result fields to be `PASS` before a HarmonyOS device-roundtrip usability claim. This improves the HarmonyOS promotion gate only; no real DevEco compile, phone/emulator import, patch export, or Mac Return Files manual pass was executed.
- The offline morning headline gate now runs the Windows static QA validator too. `scripts/morning-check.sh` runs `npm run windows:static:validate:smoke` after the dogfood and Mac manual validators, and `validate-morning-receipts.mjs` asserts the package aliases, shell gate entries, order, and generated dashboard/script wording stay aligned. `WINDOWS_STATIC_QA.md` now has a real-run Session Header (`Date/time`, reviewer, Windows browser/device, mirror build/source, transfer method, Mac import method, static-return/Mac-import gate results, elapsed time, local-file and return-file friction, biggest friction). `scripts/validate-windows-static-qa.mjs` writes `learning-companion.windows-static-qa-receipt.v1`, accepts the all-`NT` pending receipt with `canClaimWindowsStaticLoopUsable=false`, rejects missing FAIL/BLOCKED notes, rejects fully executed rows with required header fields still `TBD`, and requires both static-return contract and Mac Return Files import result fields to be `PASS` before a Windows static-loop usability claim. This improves the Windows promotion gate only; no real Windows browser run was executed.
- The offline morning headline gate runs the Mac manual QA validator. `scripts/morning-check.sh` runs `npm run mac:manual:validate:smoke` immediately after `dogfood:validate:smoke`, and `validate-morning-receipts.mjs` asserts that package alias, the shell gate entry, and the generated dashboard/script wording stay aligned. `check:morning` remains an offline fixture/contract gate: the Mac manual QA receipt is still all `NT`, `canClaimMacManualQaUsable=false`, and real Mac GUI dogfood is still Not Run.
- Mac manual QA now has the same kind of claim guard as the dogfood runbook. `MAC_MANUAL_QA.md` gains a Session Header for real-run metadata (`Date/time`, reviewer, Mac build/source, macOS version, browser/source used, native/browser gate results, elapsed time, permission prompts, native save/import friction, biggest friction), states that it is not evidence until filled from an actual Mac GUI run, and says controlled browser smoke, SwiftPM build success, and fixture receipts cannot fill rows. `scripts/validate-mac-manual-qa.mjs` writes `learning-companion.mac-manual-qa-receipt.v1`, accepts the all-`NT` pending receipt with `canClaimMacManualQaUsable=false`, rejects missing FAIL/BLOCKED notes, rejects an all-PASS table when required real-run header fields remain `TBD`, and requires native/browser gate result fields to be `PASS` before a usability claim. This improves the Mac promotion gate only; no real GUI dogfood was run.
- Morning dogfood runbook now tracks the source-first Recall decision explicitly instead of only tracking Notes. `DOGFOOD_RUNBOOK.md` adds `Save-for-recall source-return count` and `Save-for-recall Review-card count`, Mac Study Loop step 4 asks the reviewer to record whether source-return or `Review card` is used first after a source-linked card save, and Decision Rules mark repeated `Review card` first-use as possible Mac-loop friction. `validate-dogfood-runbook.mjs` now requires both Save-for-recall counters to be non-negative integers before a fully executed Mac loop can claim usability. The dashboard Dogfood Gate says to record Notes/Recall source-return counts. Mira broker was unavailable for this packet (`AUTH_EXPIRED`, request id `31d16482-9c0e-46e3-a962-16bba5882737`), so Seed `ark/seed-code-0602` provided the external review and returned `VERDICT: PASS_WITH_NOTES`; accepted constraints were parallel counters, Mac-loop validator gating, step-4 wording, decision-rule threshold, and morning receipt assertions.
- Post-save source-return hint readability now has a focused visual evidence path. `npm run smoke:post-save-hints` drives a credential-free headless Chrome app instance through source-backed Add-to-Notes and Save-for-recall states at 390x760 and 1280x720, writes a receipt to `.codex-tmp/post-save-hint-visual/receipt.json`, and stores activity-strip screenshots under `.codex-tmp/post-save-hint-visual/run-*/screenshots/`. The receipt schema is `learning-companion.post-save-hint-visual.v1`, evidence type is `CONTROLLED_HEADLESS_BROWSER_VISUAL`, and `provesRealUserDogfood=false`; it checks no horizontal overflow, visible hint text/action, no hint/button overlap, and no Activity action overflow. The latest receipt shows source-linked Recall after-save as source-first (`Open at quote` primary, `Review card` secondary). It does not prove Safari, Windows, HarmonyOS, screen-reader behavior, or human comprehension.
- Source-linked Recall-card after-save is now source-first in reading/capture contexts. Direct Save-for-recall and Recent Stack/capture-surface promotion use a distinct `reviewCardSourceResume` Activity target, with `Open at quote` / `Resume source` as the primary action and `Review card` as the secondary hint. Today question-management cards explicitly stay Review-first, because they are not the mid-reading continuation path reviewed in this slice. Mira returned `PASS_WITH_NOTES`; accepted constraints were a distinct target pane, direct+promotion coverage, shared source safety via `buildCaptureResumeSource`, unsafe promoted coverage, and no real-user intent claim.
- Source-linked Recall-card cross-session behavior now has explicit controlled browser coverage for the source-first shape. The post-save smoke verifies the source session exposes the secondary `afterCardMade` review hint, switches to a newly created session where stale controls are hidden, force-clicks the stale hidden hint button and confirms no source opens, then restores the source session and confirms the hint returns. Product behavior changed deliberately in this slice; evidence remains controlled browser smoke, not real dogfood.
- Browser smoke import-error receipt coverage is now condition-based instead of fixed-sleep based. The main browser smoke waits for mirror restore to actually prune the stale draft and reset the file input before continuing, and the bad mirror / malformed JSON / oversized inbox patch receipt checks now run in a focused async CDP block that waits for both the expected filename in `#importReceipt` and input reset. This hardens the real flake where `malformedImportReceipt` could still read the prior `bad-mirror.json` receipt. Product behavior was unchanged; this is controlled smoke reliability evidence, not real dogfood.
- Source-linked Recall-card deletion now has explicit controlled browser coverage. The post-save smoke creates a source-linked highlight card with source-first `Open at quote`, removes the backing review card from the fixture workspace, verifies the secondary Review hint hides, clicks the primary source action and the hidden hint, and confirms `Review card no longer exists` is shown with no `window.open` call. A separate missing-source fixture keeps the card but removes its source capture, and the primary source action reports `Source no longer exists` without opening a URL. Restoring the card makes the secondary `afterCardMade` hint available again.
- Recall-card source fallback now has explicit controlled browser coverage for unsafe source URLs in both direct save and promoted-save paths. The post-save smoke creates a direct capture/card after entering `javascript:alert(1)` as the source URL, and a promoted highlight/card after entering `javascript:alert(2)`. In both cases the saved session/capture source URLs are blank, the review card still exists, Activity falls back to `afterCardMade` with `Review card`, clicking the hint opens the review card, and `window.open` is not called. This is controlled Chrome smoke evidence only.
- Morning demo now labels legacy Feishu-named mirror residue instead of leaving it as an implicit no-clean caveat. When `MORNING_DEMO_SKIP_CLEAN=1` leaves `sample-feishu-mirror.json` or `sample-learning-companion-feishu-mirror.zip` in `dist/morning-demo/`, `review-start-here.html` renders `<section id="legacy-artifact-notice" data-status="stale_no_clean">`, and both `SUMMARY.json` and `EVIDENCE_TIERS.json` carry a matching `legacy_artifacts` block with `status`, `severity`, `current_outputs`, detected legacy file names, mtimes, and sha256 hashes. The current mirror outputs remain `sample-mirror.json` and `sample-learning-companion-mirror.zip`; the legacy names are `PENDING_USER_GATE` residue and must not be interpreted as current mirror evidence, Feishu live sync evidence, real dogfood, or cross-platform proof. Validator behavior is deliberate: `stale_no_clean` prints a warning and passes, while `unexpected_residue` is an error. Mira returned `PASS_WITH_NOTES`; accepted notes were runtime detection, dedicated machine-readable schema, deterministic HTML notice, validator severity handling, and this handoff pointer.
- First-run Today now makes the learning action the actual first screen. `Learning Flow` is the first Today content block, `today-summary` moves below it, and the first-run `Close the loop -> Capture first` action now lands in Quick Capture with `First capture ready` copy. The loop preview is full-width to avoid a tall narrow card, and the First Note `Other devices` route is collapsed by default as `Other devices later`, exposing the full manual phone/Windows route only after the user expands it. In-app Browser measured the first-run Learning Flow panel at 581px at 1280x720 after this pass, down from about 784px before the ordering/compactness changes, with no horizontal overflow. This is still controlled UI evidence, not real dogfood.
- After the first ordinary capture, Today no longer calls the loop `Clear` while that capture is still missing from Notes and Review. `Close the loop` now shows `Needs next step`, `Choose next` opens the capture detail, and `Next Move` mirrors the same decision. Adding that capture to Notes returns the isolated smoke fixture to `Clear`, so the app now distinguishes "saved locally" from "made durable enough to move on." Mira returned `PASS_WITH_NOTES`; accepted follow-up softened the wording away from a hard gate and added the "Add to notes -> Clear" transition smoke. Parked questions already outrank this capture decision in code, so Mira's ordering concern was treated as a misread rather than a product change.
- The capture-decision predicate is now narrower and more visible: only ordinary captures whose next-step cue is `Choose next` count as durable-decision debt, while takeaway/support captures no longer force the loop out of `Clear`. When a capture does need a durable decision, its detail/stack row gets `needs-durable-decision`, and only the `Add to notes` / `Save for recall` buttons get the `capture-decision-button` styling. This absorbs Mira's "predicate too broad" warning without adding a fake keep-support state.
- Today loop priority now has an explicit parked-vs-capture-decision browser fixture: a newer ordinary capture can keep its `needs-durable-decision` marker, but one parked question still owns `Close the loop` and `Next Move`. First-run loop copy is shorter and keeps the boundary visible: `Save the first point, then decide whether it belongs in Notes, Review, or a later phone/Windows pass.` The collapsed `Other devices later` row now uses a two-row summary layout so its label, copy, and `+` affordance do not collide in the narrow Today side panel. Mira returned `PASS_WITH_NOTES`; accepted follow-up added quote-only and unlinked-answer negative cases so support captures do not regress into `Needs next step`.
- Focus Sidecar now uses action-first rail copy for the capture lane. Empty sidecar says `Capture first point / Focus field`; after the first capture it flips to `Capture next point / Focus field`; draft state remains `Draft waiting / Resume`. A new controlled agent smoke, `npm run agent:study-loop`, drives a source-backed sidecar path through first capture -> durable Notes decision -> open question -> linked answer -> clear loop, then writes `.codex-tmp/agent-study-loop-smoke/receipt.json`. The receipt schema is `learning-companion.agent-study-loop-smoke.v1`, evidence type is `CONTROLLED_AGENT_BROWSER_SMOKE`, and `provesRealUserDogfood=false`; this is a focused regression receipt, not a human dogfood claim.
- Quick Capture heading now makes the ordinary capture path more dominant. Empty `No draft` status is hidden until a real local draft exists, the Focus Brief capture CTA is `Start typing` instead of a second green `Capture`, secondary actions are visible as `Save for recall` and `Blank` with recall-oriented title/aria copy, and the starter row label changed from `Start draft` to `Write as`. In-app Browser visual QA showed `Capture`, `Save for recall`, and `Blank` stay on one row at the current sidecar-ish width, `Write as` is visible, and there is no horizontal overflow. Seed returned `PASS_WITH_NOTES`; accepted concrete points were dual-primary Capture confusion and Card/Cloze jargon.
- Quick Capture no-source empty state now stops pretending the learner is fully `Ready`. With no source title/URL, Activity says `Link source or jot loose thought`, its action is `Set source`, and clicking it focuses the Source URL field; the capture context intent says `No source` with copy explaining captures still work but are not resumable without an anchor. Source-linked sessions still show `Ready to capture` and route Activity to Quick Capture. Browser smoke asserts the no-source Activity click lands on `sourceUrl`; explicit Quick Capture/rail entry still focuses the capture field so an intentional loose thought remains possible. Mira returned `PASS_WITH_NOTES`; accepted follow-up documented the two-intent split in code and kept the post-click focus assertion. Rejected: blindly redirecting every `focusQuickCapture()` no-source caller to Source URL, because that would break explicit `Focus field`/loose-thought entry.
- Static Review/Inbox return hints now explain where a browser-saved return JSON may be found without implying the app writes to Downloads by default. The shared hint leads with the primary action (`Move it to Mac, then import or paste it from Today > Return Files`), then separates saved-file lookup from no-file fallback, softens HarmonyOS to `Files app's Downloads folder`, and keeps Feishu Drive only as a manual carrier with no live sync. Mirror export filenames are now generic (`learning-companion-mirror.json` / `learning-companion-mirror.zip`) so the static package no longer reads as Feishu-specific sync. README also states that static Review/Inbox files must be moved back to Mac manually and that live Feishu sync is not verified. Mira returned `PASS_WITH_NOTES`; accepted follow-up made the location language conditional, added the other-device fallback, de-emphasized Feishu as manual-only, loosened static/browser assertions away from one brittle sentence, then later pushed for action-first copy, fallback separation, and softer HarmonyOS wording. Deferred: real Windows/Harmony visual verification, rendered in-app screenshot, and real manual device loop.
- After a capture/highlight is saved for recall from a reading/capture surface, source-linked cards now mirror Notes' source-first continuation pattern. The Activity main action opens the source (`Open at quote` when a text fragment is available, `Resume source` otherwise) through `reviewCardSourceResume`; the secondary hint is `afterCardMade` / `Review card`. No-source, unsafe-source, and Today question-management card creation stay Review-first. Browser smoke proves primary source resume opens a separate tab with `noopener,noreferrer`, returns the app to Captures, focuses `thoughtInput`, pulses Quick Capture, keeps secondary Review one click away, and fails safely for deleted cards, missing source captures, stale cross-session controls, direct unsafe sources, and promoted unsafe sources. Mira returned `PASS_WITH_NOTES`; accepted follow-up made the target pane distinct and kept scope claims to controlled local evidence. Deferred: telemetry, real Mac dogfood, Safari/manual visual QA, Windows/HarmonyOS behavior, screen-reader audit, and human comprehension.
- Source resume for ordinary article/document captures now uses `#:~:text=` when there is no valid timestamp and the capture has source quote text. `buildSourceTextFragmentUrl()` keeps the existing video timestamp behavior for YouTube/Bilibili/Vimeo, skips unsafe URLs and direct `.pdf` links, preserves existing hashes as `#section:~:text=...`, avoids duplicate text directives, rejects too-short or punctuation-only anchors, and does not use linked-answer generated quote text as a source anchor. `buildResumeSource()` returns `hasTextFragment` for future hint/telemetry separation, and capture-specific resume passes the current capture explicitly so a newer capture cannot steal the anchor. `npm run smoke:text-fragment` proves headless Chromium scrolls a local below-the-fold target into view from `#:~:text=...`; `npm run smoke`, `npm run smoke:source-resume`, and `npm run smoke:browser` cover URL construction plus Activity/review-card source resume paths. Mira returned `PASS_WITH_NOTES`; accepted follow-up added sanitizer edge coverage, `.pdf` skip, `hasTextFragment`, explicit capture override coverage, and the real Chromium text-fragment smoke. Not run: Safari fallback/manual visual QA and real Windows/HarmonyOS browser behavior.
- Text-fragment source resume is now visible in Activity as `Open at quote` instead of generic `Open source`/`Resume source` when `hasTextFragment=true`. This applies to ordinary capture, annotated highlight, review-card-created, and review-queue-clear hints; timed video and no-fragment source paths keep their older labels. Aria labels intentionally weaken the promise as `jump ... if supported`, because browsers may fall back to opening the page without locating the quote. Browser smoke covers the new kind routing, including the regression where annotated highlights briefly returned with `View capture` instead of `View highlight`, plus a thought-only source capture negative case that must stay `Open source` with a bare URL. Mira returned `PASS_WITH_NOTES`; accepted follow-up weakened aria copy and added the no-fragment negative smoke. Not run: in-app Browser form-entry spot check, because the Browser backend input APIs failed without the virtual clipboard and page evaluate is read-only; headless smoke remains the behavior evidence.
- Capture detail now gives ordinary captures a tighter destination cue before the durable decision buttons: `Notes: connect ideas · Recall: remember later`. This guide only appears on captures where `captureNeedsDurableDecision()` is true and only in the detail view; Mira pushed back on showing it in Recent Stack, so the stack keeps its scan-friendly next-step text without repeated guide pills. `Save for recall` and promoted `Review` buttons now carry title/aria text that explains the action without exposing implementation details. Browser smoke covers Today `Choose next` -> capture detail guide, decision button title/aria, Recent Stack negative coverage for promoted/quote-only/ordinary rows, and the no-guide stack policy. In-app Browser read-only reload of `http://127.0.0.1:5173/` showed the page loads with no error/warning logs and no horizontal overflow; the current localStorage state had one visible decision guide. Mira returned `PASS_WITH_NOTES`; accepted follow-up changed the copy from `synthesize/practice later` to `connect ideas/remember later`, removed the guide from Recent Stack, and simplified recall aria/title copy. Not run: real dogfood proof that this reduces hesitation, 320-360px visual screenshot, and learn-once dismissal.
- Adding a capture to Notes now keeps the reading loop alive instead of leaving Notes as a dead end. For new or updated note blocks with a resolvable source, Activity's main action is now the learning-continuation action: source-only notes show `Open source`, text-fragment notes show `Open at quote`, and timed/video notes show `Resume source`. `View note` is demoted to the next hint so the generated block remains inspectable without becoming the primary path. If the user only opens an already-current note block, the main action remains `View note` and no new source hint is emitted. Resuming from a note opens the source in a new tab, keeps Quick Capture ready, then shows `Focus field` plus a `View note` hint; the detail says `The note is saved. Keep reading; capture the next point when it lands.` Browser smoke covers new note -> `#:~:text=...`, timed note -> `t=201s`, stale `Update note` -> source-first main action, sourceResume aria/title copy, and source-resumed `Focus field`. Mira returned `PASS_WITH_NOTES` across the note-resume and source-first reviews; accepted follow-up neutralized the resume copy, distinguished update text, added timed-source coverage, boolean-normalized `didUpdate`, hardened dynamic hint overrides with kind validation plus text/action/aria length limits, and changed the post-resume main action from `Capture` to `Focus field`. Seed/Doubao independently pushed for source-first after noting sidecar would otherwise show `Exit + View note`. Recall-card source-first follow-up is now completed for reading/capture contexts. Deferred: telemetry for repeat Add-to-Notes, Safari/Harmony/Windows text-fragment behavior, and real dogfood confirmation.
- The morning dogfood runbook now captures the Notes/source-return decision quantitatively instead of leaving it as an open opinion. The session header includes `Add-to-Notes source-return count` and `Add-to-Notes View-note count`; Mac Study Loop step 5 asks the reviewer to insert a capture into Notes, then record whether the source-return main action or `View note` is used first. Since source-return is now the main action, the decision rule watches for the opposite failure: if `View note` is used three or more times because saved-note confirmation feels weak, record it as Mac loop friction and strengthen confirmation or reconsider the main action. The dogfood validator requires those counts to be non-negative integers once Mac rows are actually executed, while generated `NT` runbooks still stay `PENDING_USER_GATE`. Verified with `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning`, `npm run dogfood:validate:smoke`, and `npm run morning:receipts`.
- HarmonyOS scaffold now makes the phone/Mac return-patch asymmetry explicit. Index says `Import reader / Return to Mac`, warns return patches are Mac-bound only, and the scaffold import service returns `PATCH_IMPORT_NOT_SUPPORTED_ON_READER` for mobile inbox or review-progress patch schemas instead of a generic unsupported-data error. This aligns the ArkTS scaffold with the JS import boundary; it is still scaffold evidence, not a real device run.
- README now starts with verified status instead of product ambition: web MVP plus minimal Mac shell scaffold are runnable; phone/Windows are static mirror + manual return files; Feishu Drive is only a possible manual carrier; HarmonyOS is scaffold/prototype, not device-verified. This absorbs Seed's finding that the prior README implied Feishu sync and HarmonyOS app readiness contradicted the UI/docs boundary.
- First-run Learning Flow now shows the full route before the first saved note: `Read source`, `Capture on Mac`, and a pending `Close the loop` step. The loop step says `Pending - After first capture`, explains Notes/Review/phone-Windows return files become useful after saving the first point, and routes `Capture first` back to Quick Capture. This makes the route visible without claiming the loop is already clear.
- Device Flow now includes a default-open `Manual round trip` guide before export/import actions. It spells out Mac -> Windows ZIP extraction and `index.html`, Mac -> Harmony `workspace.json` import plus Mac-ward-only return patches, Device -> Mac return JSON names (`learning-companion-inbox-patch-*.json` and `learning-companion-review-progress-patch-*.json`), browser download fallback handling, Mac-side Import/Paste, and the boundary that Feishu Drive is only a manual file carrier, not verified live sync.
- Morning review dashboard now opens with a `Morning Dogfood Gate` section before the artifact grid. It tells the reviewer to use the real app beside a real lesson, spend 15 minutes on rows 1-6 of `DOGFOOD_RUNBOOK.md`, record actual friction, leave untouched rows as `NT`, and run `npm run dogfood:validate -- --runbook dist/morning-demo/DOGFOOD_RUNBOOK.md --out .codex-tmp/dogfood-runbook/real-run-receipt.json` before claiming the route usable. The first card now states `Generated status: NOT RUN` and `0 PASS / 11 NT / usable=false`, so the generated dashboard cannot be mistaken for real dogfood evidence. Mira returned `PASS_WITH_NOTES`; accepted follow-up pins the gate as the first dashboard section, checks the validate command against the wired npm script, and makes the unrun status visible. This is a review-path improvement, not dogfood evidence.
- Morning review materials now include `AGENT_STUDY_LOOP_SMOKE.md`, a generated command note for `npm run agent:study-loop`. The dashboard, artifact row, and Stage Matrix use the canonical label `controlled-agent-browser-smoke`, and `npm run morning:receipts` asserts the boundary strings `CONTROLLED_AGENT_BROWSER_SMOKE`, `provesRealUserDogfood=false`, `not real dogfood`, and `no Mac/Windows/HarmonyOS/Feishu/native picker/file movement coverage`. This exposes the fast sidecar-loop regression path for tomorrow without letting it fill `DOGFOOD_RUNBOOK.md` or `MAC_MANUAL_QA.md`.
- Quick Capture source context now mirrors source-drift risk on the source chip itself, not only in the draft status. When a local draft began on one source and the current session source changes, the chip gets a warn style plus title/aria copy naming the draft origin and current source; restoring the original source clears the warning. Browser smoke also clears fixture draft state before ordinary mobile layout checks so normal source-chip assertions stay honest.
- Capture delete undo now exposes the expiry instead of a vague `Undo 10s`: the Activity button reads `Undo delete (Ns)`, its title/aria names the delete recovery window and remaining seconds, it hides instead of showing a zero-second action, and a new autosave explains `Undo expired` because the recovery point was replaced. Mira returned `PASS_WITH_NOTES`; accepted fixes covered label/aria/save-invalidation smoke. Deferred: review-card undo and local counters, because they expand state surface without current dogfood evidence.
- Capture note actions now carry state-specific title and aria metadata in both Capture detail and Recent Stack. The missing/current/stale states still render `Add to notes`, `View in Notes`, and `Update note`, but smoke now pins the accessible action meaning so the three-state path is less ambiguous in sidecar and detail flows.
- Capture note blocks now carry a capture-content fingerprint, so generated notes can distinguish three states without overwriting user annotations: missing shows `Add to notes`, matching fingerprint shows `View in Notes`, and stale fingerprint shows `Update note`. Existing legacy blocks without fingerprints still fall back to the old generated-block comparison. Browser smoke verifies that user text changes inside the generated note block keep `View in Notes`, while a stale fingerprint shows `Update note` and returns to `View in Notes` after refresh.
- Capture detail now shows the same low-noise next-step cue as Recent Stack. When Activity opens a saved capture, the detail card itself tells the user whether to add to Notes, save for recall, keep reading, or review the existing card; browser smoke verifies the ordinary `View capture` path lands on a pulsed detail card with the expected cue.
- Capture detail and Recent Stack now make the durable choice explicit: ordinary captures say `Choose next: add to Notes for synthesis, or save for recall`, while already-noted captures say `In Notes · keep reading, or save for recall practice`. Capture detail no longer shows a disabled `Card` for promoted captures; it exposes a live `Review` button that opens the same review card path as Recent Stack. Mira returned `PASS_WITH_NOTES`; accepted fixes neutralized the Notes/Recall wording and added browser smoke proving the detail `Review` button opens the card.
- Capture delete actions are now visually de-weighted as `tertiary danger` buttons while keeping red semantics, confirmation, and linked-card wording. The linked-card delete title/aria now says it deletes after confirmation, and browser smoke verifies the class, title, aria, and existing cancel/delete/undo flows.
- Return Files now stages copied or selected inbox/review return JSON as an in-memory preview before applying it. Device Flow shows parsed deltas, `Apply Return File`, and `Discard`; workspace and imported patch ids stay unchanged until Apply. Mira returned `PASS_WITH_NOTES`; accepted fixes tightened preview deltas, added a workspace-fingerprint refresh guard before Apply, and added smoke for re-paste replacement, discard clearing, and file-picker batch preview before Apply.
...
- no-source first-open Today now makes `Paste source` the primary First Note action, using the existing user-initiated clipboard source parser from the top Learning Flow surface. `Set source manually` remains as a tertiary fallback, `Jot loose thought` stays available for unanchored notes, and the capture step remains `After source`. After a successful source paste, Activity now exposes `Capture` and targets Quick Capture instead of leaving a generic details action. Mira returned `PASS_WITH_NOTES`; accepted fixes de-weighted the manual source fallback, tightened the copy, and added browser smoke for non-URL clipboard fallback plus proof that manual source setup does not read the clipboard. Current evidence is controlled browser smoke plus source-linked in-app spot check, not a real fresh-profile dogfood run.
- Plain source-backed quote+thought captures now show `View capture` as the Activity action plus a source next-step hint after save. This keeps the just-saved capture inspectable for Notes/Recall while the hint keeps the reading loop moving without overclaiming playback state: bare source URLs show `Open source`, timestamped captures show `Resume source`. The existing full browser smoke now asserts the action and hint appear, and `npm run smoke:source-resume` verifies the focused browser paths: view the saved capture, bare source open, source-less capture negative case, timestamped YouTube resume with `t=42s`, separate tab target with `noopener,noreferrer`, return to Quick Capture, and focus the thought field for the next point.
...
- For the Windows static-QA validator slice, local verification passed: `bash -n scripts/morning-check.sh`, `node --check scripts/validate-windows-static-qa.mjs`, `node --check scripts/build-morning-demo.mjs`, `node --check scripts/validate-morning-receipts.mjs`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning` -> `morning_demo_ok`, `npm run morning:receipts` -> expected `legacy_artifacts=stale_no_clean` warning then `morning_receipts_ok`, `npm run windows:static:validate:smoke` -> pending receipt with `rows=10`, `nt=10`, `allRowsExecuted=false`, `canClaimWindowsStaticLoopUsable=false`, negative Windows validator checks under `.codex-tmp/windows-static-qa/` exited nonzero as expected for all-PASS rows with header fields still `TBD` and for a `FAIL` row without a QA note, and `MORNING_DEMO_SKIP_CLEAN=1 LC_KEEP_CHECK_ARTIFACTS=1 npm run check:morning` -> `morning_offline_check_ok` including `Windows static QA validator`. This is evidence-surface work only; no real Windows browser run was executed.
- For the morning gate integration slice, local verification passed: `bash -n scripts/morning-check.sh`, `node --check scripts/build-morning-demo.mjs`, `node --check scripts/validate-morning-receipts.mjs`, `node --check scripts/validate-mac-manual-qa.mjs`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning` -> `morning_demo_ok`, `npm run morning:receipts` -> expected `legacy_artifacts=stale_no_clean` warning then `morning_receipts_ok`, `npm run mac:manual:validate:smoke` -> `rows=27`, `nt=27`, `canClaimMacManualQaUsable=false`, `npm run dogfood:validate:smoke` -> `rows=11`, `nt=11`, `canClaimMacDogfoodUsable=false`, `MORNING_DEMO_SKIP_CLEAN=1 LC_KEEP_CHECK_ARTIFACTS=1 npm run check:morning` -> `morning_offline_check_ok`, and `git diff --check`. `npm run check:morning:browser` first failed in the sandbox with `listen EPERM 127.0.0.1`; the approved rerun returned `smoke_browser_ok` and `morning_browser_check_ok`. This is controlled local/offline/browser evidence only; it does not fill dogfood or Mac manual QA rows.
- For the Mac manual QA validator slice, `npm run check:morning:native` first failed inside the Codex sandbox with SwiftPM `sandbox_apply: Operation not permitted`; the approved outside-sandbox rerun returned `morning_native_check_ok`. External review did not produce a valid verdict: Mira broker returned `AUTH_EXPIRED`, request id `0ce255d5-ab10-4a4c-b8dc-263413357492`; Seed `ark/seed-code-0602` hit `Error: Reached max turns (14)` and is not evidence; Coco first failed DNS for `aime.bytedance.net`, then after network escalation returned `401 Unauthorized: invalid token`. Local verification passed: `node --check scripts/validate-mac-manual-qa.mjs`, `node --check scripts/build-morning-demo.mjs`, `node --check scripts/validate-morning-receipts.mjs`, `node --check scripts/validate-dogfood-runbook.mjs`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning` -> `morning_demo_ok`, `npm run morning:receipts` -> expected `legacy_artifacts=stale_no_clean` warning then `morning_receipts_ok`, `npm run mac:manual:validate:smoke` -> pending receipt with `rows=27`, `nt=27`, `allRowsExecuted=false`, `canClaimMacManualQaUsable=false`, `npm run dogfood:validate:smoke` -> pending dogfood receipt, `git diff --check`, and `npm run smoke` -> `smoke_web_ok`. Negative Mac QA validator checks under `.codex-tmp/mac-manual-qa/` exited nonzero as expected for all-PASS rows with header fields still `TBD`, and for a `FAIL` row without a QA note. Real Mac GUI QA remains Not Run.
- For the dogfood Recall source-return counter slice, Mira broker did not produce a verdict: `.mira-review/dogfood-recall-source-return-counters-20260604.md` returned `ok=false`, `error_code=AUTH_EXPIRED`, `error_stage=provider`, `cleanup_succeeded=false`, `logid_present=false`, request id `31d16482-9c0e-46e3-a962-16bba5882737`. External review switched to Seed (`ark/seed-code-0602`) using `.codex-tmp/seed-dogfood-recall-counters-task.jsonl`; Seed returned `VERDICT: PASS_WITH_NOTES` in `.codex-tmp/seed-dogfood-recall-counters/dogfood-recall-counters.stdout.txt`. Accepted constraints: add `Save-for-recall source-return count` and `Save-for-recall Review-card count`, require both counters for a fully executed Mac loop, update Mac Study Loop step 4, add a Recall friction rule, and pin the generated fields in `morning:receipts`. Verification passed: `node --check scripts/build-morning-demo.mjs`, `node --check scripts/validate-dogfood-runbook.mjs`, `node --check scripts/validate-morning-receipts.mjs`, `git diff --check`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning` -> `morning_demo_ok`, `npm run morning:receipts` -> `morning_receipts_warning legacy_artifacts=stale_no_clean files=sample-feishu-mirror.json,sample-learning-companion-feishu-mirror.zip` then `morning_receipts_ok`, and `npm run dogfood:validate:smoke` -> pending receipt with `pass=0`, `nt=11`, `macLoopExecuted=false`, `canClaimMacDogfoodUsable=false`. A negative validator check using `.codex-tmp/dogfood-runbook/recall-counter-required.md` exited nonzero as expected and wrote `.codex-tmp/dogfood-runbook/recall-counter-required-receipt.json`, with errors requiring both Save-for-recall counters before a six-row Mac loop can claim usability. This is evidence-surface work only; no real Mac dogfood was run.
- For the Recall-card source-first after-save slice, Mira broker returned `PASS_WITH_NOTES` for `.mira-review/recall-after-save-source-first-20260604.md` with `ok=true`, `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, request id `695d015b-df57-42ea-a5ee-4c260aad9428`, and status verdict `PASS_WITH_NOTES`. The local parser did not accept the raw response block (`valid=false`) because the broker response used a wrapper before the verdict; the local response artifact was sanitized to remove that wrapper while preserving `VERDICT: PASS_WITH_NOTES` and the notes, then `mira_review.py parse` returned `valid=true`, `accepted=true`, `source=heading`. Accepted constraints: use distinct `reviewCardSourceResume`, apply source-first to direct Save-for-recall and reading/capture promotion, keep Today question-management cards Review-first, share source safety through `buildCaptureResumeSource`, add unsafe promoted coverage, and keep evidence claims narrow. Verification passed: `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/post-save-hint-visual-check.mjs`, `git diff --check`, and `npm run smoke:browser` -> `smoke_browser_ok`. `npm run smoke:post-save-hints` first failed under sandbox with `listen EPERM: operation not permitted 127.0.0.1`; rerunning the same script with approved local-server permission returned `post_save_hint_visual_ok /Users/bytedance/Documents/Codex/learning-companion/.codex-tmp/post-save-hint-visual/receipt.json`. The refreshed visual receipt has `result=PASS`, `createdAt=2026-06-04T15:22:26.004Z`, `evidenceType=CONTROLLED_HEADLESS_BROWSER_VISUAL`, `provesRealUserDogfood=false`, and screenshots under `.codex-tmp/post-save-hint-visual/run-1780586543950/screenshots/`. This proves controlled local Chrome behavior only; Safari, Windows, HarmonyOS, screen-reader behavior, human comprehension, telemetry, and real dogfood remain Not Run.
- For the legacy no-clean mirror artifact boundary slice, Mira broker returned `PASS_WITH_NOTES` for `.mira-review/stale-no-clean-artifact-boundary-20260604.md` with `ok=true`, `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`; after removing the local diagnostic wrapper from the ignored response artifact, `mira_review.py parse` returned `verdict=PASS_WITH_NOTES`, `valid=true`, `accepted=true`, `source=heading`. Accepted notes: runtime detection, dedicated `legacy_artifacts` block, stable `legacy-artifact-notice` HTML, warning/error validator split, current-output existence checks, no overlap between current and legacy names, and handoff pointer. Verification passed: `node --check scripts/build-morning-demo.mjs`, `node --check scripts/validate-morning-receipts.mjs`, `git diff --check`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning` -> `morning_demo_ok`, and `npm run morning:receipts` -> `morning_receipts_warning legacy_artifacts=stale_no_clean files=sample-feishu-mirror.json,sample-learning-companion-feishu-mirror.zip` followed by `morning_receipts_ok`. `rg` confirmed `legacy_artifacts` and `stale_no_clean` in both `dist/morning-demo/SUMMARY.json` and `dist/morning-demo/EVIDENCE_TIERS.json`, and `legacy-artifact-notice` in `dist/morning-demo/review-start-here.html`. This is labeling/evidence-boundary work only; the old files were not deleted and no real Windows/HarmonyOS/Feishu/dogfood claim was added.
- For the no-source Quick Capture slice, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-web.mjs`, `node --check scripts/agent-study-loop-check.mjs`, `git diff --check`, `npm run smoke`, `npm run smoke:browser`, and `npm run agent:study-loop` passed. `npm run smoke` returned `smoke_web_ok`, `npm run smoke:browser` returned `smoke_browser_ok`, and `npm run agent:study-loop` returned `agent_study_loop_check_ok .codex-tmp/agent-study-loop-smoke/receipt.json`. In-app Browser reload of `http://127.0.0.1:5173/` showed `Capture`, `Save for recall`, and `Blank` on one row, `Write as` visible, no horizontal overflow at 1280x720, and no browser error/warning logs. The in-app `window.learningCompanionNative` bridge was not visible in that context, so bridge readiness is proven only by headless browser smoke.
- For the static Review/Inbox return-location slice, `node --check apps/companion-web/src/model.js`, `node --check scripts/static-return-loop-check.mjs`, `node --check scripts/smoke-browser.mjs`, `git diff --check`, `npm run smoke`, `npm run smoke:browser`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning`, `npm run check:static-return`, and `MORNING_DEMO_SKIP_CLEAN=1 LC_KEEP_CHECK_ARTIFACTS=1 npm run check:morning` passed. The first `npm run smoke:browser` attempt failed because the browser smoke still expected the old `On a phone: if the file is hard to find` sentence; the assertion was corrected to the new conditional Windows/Harmony/manual-carrier contract and the rerun returned `smoke_browser_ok`. `npm run check:static-return` also initially read a stale `dist/morning-demo` bundle before regeneration; after `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning`, it returned `static_return_loop_ok`. The latest static-return receipt path is `.codex-tmp/static-return-loop-check/static-return-loop-1780571421891/receipt.json`. Full morning gate returned `morning_offline_check_ok` and the dogfood validator stayed `PENDING_USER_GATE` with `macLoopExecuted=false` and `manualDeviceLoopExecuted=false`.
- For the after-card next-step/source-resume slice, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-web.mjs`, `git diff --check`, `npm run smoke`, and `npm run smoke:browser` passed. `npm run smoke` returned `smoke_web_ok`; `npm run smoke:browser` returned `smoke_browser_ok`. The first browser-smoke attempt failed because the expected prompt was incorrectly assumed to be the quote; the real review prompt came from the added thought (`Recall the point behind: ...`), and the test was corrected without changing product logic. The follow-up browser-smoke attempt failed because I expected resume to keep `focusMode=review`; the real behavior was `focusMode=capture`, which is correct for returning to source/Quick Capture, so the assertion was corrected.
- For the source-resume text-fragment slice, `node --check apps/companion-web/src/model.js`, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-text-fragment-browser.mjs`, `node --check scripts/smoke-web.mjs`, `node --check scripts/smoke-source-resume.mjs`, `node --check scripts/smoke-browser.mjs`, `git diff --check`, `npm run smoke:text-fragment`, `npm run smoke`, `npm run smoke:source-resume`, and `npm run smoke:browser` passed. `npm run smoke:text-fragment` returned `smoke_text_fragment_browser_ok` after the first implementation was corrected: local port binding failed under sandbox for the new npm script, then a `file://` version proved Chrome strips text-fragment directives from JS-visible `location.hash`, so the final assertion checks actual scroll position and target viewport placement instead of hash equality. The in-app Browser spot check on `http://127.0.0.1:5173/` showed a source-backed capture produces `afterCaptureSavedSourceLinked` / `Open source`; the in-app single-tab mode did not expose a new-tab URL, so URL/open behavior remains covered by headless browser smoke.
- For `2ac05f7`, `e231bb1`, `9211041`, and `168c382`, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-web.mjs`, `git diff --check`, `npm run smoke`, and `npm run smoke:browser` passed. `npm run smoke` returned `smoke_web_ok`; `npm run smoke:browser` returned `smoke_browser_ok`. In-app Browser reload of `http://127.0.0.1:5173/` confirmed `Learning Flow` is first, `today-summary` is second, the first-run panel is 581px high at 1280x720, `Other devices later` is collapsed, the hidden Phone/Windows action is not rendered until expansion, and there is no horizontal overflow. During this pass an old download-count assertion in `scripts/smoke-browser.mjs` was tightened from total-directory-count equality to "blocked review save creates no new review-progress patch", because the previous count check was unstable and did not match the guard's real intent.
- For `90b10a4` and `45d6fed`, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-web.mjs`, `git diff --check`, `npm run smoke`, and `npm run smoke:browser` passed. Browser smoke uses an isolated no-due/no-question workspace to verify a first ordinary capture produces `Needs next step`, `Choose next`, and a capture-tone loop; the action opens the capture detail; after clicking `Add to notes`, Today returns to `Close the loop: Clear`. This proves the controlled transition only; it is not a real lesson run.
- For `c386d7a`, the same syntax checks, `git diff --check`, `npm run smoke`, and `npm run smoke:browser` passed. Browser smoke now verifies the opened capture detail has `needs-durable-decision`, exactly two decision buttons (`Add to notes`, `Save for recall`), and that a Takeaway-only capture stays at `Close the loop: Clear`. In-app Browser reload of the current user page showed no decision rows in the live localStorage state and no horizontal overflow; that is a read-only sanity check, not the isolated decision fixture.
- For `bb0ad77`, `5522924`, and `836846d`, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-web.mjs`, `git diff --check`, `npm run smoke`, and `npm run smoke:browser` passed. Browser smoke now proves parked questions outrank a newer ordinary capture decision while the newer capture still retains its durable-decision marker; quote-only highlights and unlinked answers stay `Close the loop: Clear` without detail/stack decision styling. In-app Browser reload of `http://127.0.0.1:5173/` after the copy/layout slice measured `Learning Flow` at 540px, collapsed `Other devices later` at 60px, summary at 45px, and no horizontal overflow at 1280x720. This is controlled browser evidence, not real Mac dogfood.
- For `c63d4a0`, `993c7ef`, and `e67b4f4`, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-web.mjs`, `node --check scripts/agent-study-loop-check.mjs`, `git diff --check`, `npm run smoke`, `npm run smoke:browser`, and `npm run agent:study-loop` passed. The latest agent receipt at `.codex-tmp/agent-study-loop-smoke/receipt.json` records `result=PASS`, `elapsedMs=1632`, `evidenceType=CONTROLLED_AGENT_BROWSER_SMOKE`, `provesRealUserDogfood=false`, and `observed.finalCounts` with `captures=4`, `openQuestions=0`, `closedQuestions=1`, `answers=1`, `notesHasFirstCapture=true`; caveats still say headless Chrome, non-human dogfood, no HarmonyOS/Windows/Feishu/native-picker/file-movement coverage, and no background Downloads scan. The latest run completed without a sandbox retry.
- For `993322f`, `node --check scripts/smoke-harmony-scaffold.mjs`, `git diff --check`, and `npm run smoke:harmony` passed. `npm run smoke:harmony` returned `smoke_harmony_schema_ok` and `smoke_harmony_scaffold_ok`. A direct `node --check` attempt on `.ets` is not a valid check path because Node does not understand the ArkTS extension; no DevEco compile or HarmonyOS device run is claimed.
- For `81f518f`, `git diff --check` passed. A repo text scan found the README no longer contains the prior `with Feishu as the first cross-device sync layer`, `Feishu sync: ...`, or `HarmonyOS app: mobile...` claims; remaining Feishu/Harmony hits are boundary statements such as no live sync, fixture-only, scaffold-only, or device behavior not proven.
- For `6b78078`, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-web.mjs`, `git diff --check`, `npm run smoke`, and `npm run smoke:browser` passed. Browser smoke now pins first-run flow steps as `source`, `capture`, `loop`; checks `Pending - After first capture` with Notes/Review/phone-Windows copy; verifies the Device Flow guide is default-open before export/import buttons; verifies guide text includes ZIP extraction, Harmony patch asymmetry, return filename patterns, no auto-scan Downloads, and Feishu manual-carrier copy; and checks the guide at 1024px, 620px, and 360px without horizontal overflow. In-app Browser read-only reload of `http://127.0.0.1:5173/` showed the three first-run steps and no horizontal overflow at width 1265.
- For `97c230a`, `8c1813f`, and `70420ad`, `node --check scripts/build-morning-demo.mjs`, `node --check scripts/validate-morning-receipts.mjs`, `git diff --check`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning`, `npm run smoke`, `npm run morning:receipts`, `npm run dogfood:validate:smoke`, and `npm run demo:morning:serve:smoke` passed across the gate slices. The latest generated `dist/morning-demo/review-start-here.html` contains the `Morning Dogfood Gate` plus `Generated status: NOT RUN` / `0 PASS / 11 NT / usable=false`. Mira returned `PASS_WITH_NOTES`; accepted follow-up checks the gate is the first dashboard section, that the dashboard validate command matches `dogfood:validate`, and that the generated dashboard labels fixture-only status. A fixed `npm run demo:morning:serve -- --port 5174` check hit local port-binding sandbox `EPERM`; no approval was requested, and the random-port serve smoke remains the executed HTTP proof.
- For `7601f61`, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-web.mjs`, `git diff --check`, `npm run smoke`, `npm run smoke:source-resume`, and `npm run smoke:browser` passed. In-app Browser read-only reload of `http://127.0.0.1:5173/` showed Learning Flow and Quick Capture visible with no horizontal overflow at width 1265.
- For `e438571`, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-web.mjs`, `git diff --check`, `npm run smoke`, `npm run smoke:source-resume`, and `npm run smoke:browser` passed. In-app Browser read-only reload of `http://127.0.0.1:5173/` showed Learning Flow and Quick Capture visible with no horizontal overflow at width 1265. Mira packet v1 was rejected by the local broker sanitizer as `SECRET_DETECTED`; reduced v2 returned `PASS_WITH_NOTES`, accepted for undo label/aria/save-invalidation, with review-card undo and counters deferred.
- For `49e887a`, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-web.mjs`, `git diff --check`, `npm run smoke`, `npm run smoke:source-resume`, and `npm run smoke:browser` passed. In-app Browser read-only reload of `http://127.0.0.1:5173/` showed Learning Flow and Quick Capture visible with no horizontal overflow at width 1265.
- For `46a57cc`, `77e5385`, and `85deda5`, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `git diff --check`, `npm run smoke`, `npm run smoke:browser`, and `npm run smoke:source-resume` passed. Mira reviewed the capture-detail next-step slice as `PASS_WITH_NOTES`; accepted follow-up fixed stale detection with capture fingerprints and added a smoke case where user edits inside the generated block do not force `Update note`.
- For `35a9fdc` and `6e078c1`, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-source-resume.mjs`, `git diff --check`, `npm run smoke`, `npm run smoke:source-resume`, and `npm run smoke:browser` passed. In-app Browser read-only reload of `http://127.0.0.1:5173/` showed Learning Flow and Quick Capture visible with no horizontal overflow. The current user page did not have a visible capture row, so `Add to notes` visibility is covered by controlled browser smoke rather than that read-only page.
- `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning` returned `morning_demo_ok`; `npm run dogfood:validate:smoke` returned a valid pending receipt with `rows=11`, `nt=11`, `pass=0`, `canClaimMacDogfoodUsable=false`, and `canClaimManualDeviceLoopUsable=false`; `npm run demo:morning:serve:smoke` returned `morning_demo_server_smoke_ok`. This proves the dogfood gate is present and still refuses fixture-only usability claims, not that real dogfood has been executed.
...
- For `8d6b412` plus the follow-up source-only mobile browser, static Inbox source-context, static `noscript`, and phone-friendly return transport coverage, `node --check apps/companion-web/src/model.js`, `node --check scripts/smoke-web.mjs`, `node --check scripts/static-return-loop-check.mjs`, `node --check scripts/smoke-browser.mjs`, `git diff --check`, `npm run smoke`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning`, `npm run check:static-return`, and `npm run smoke:browser` passed. The latest static-return receipt at `.codex-tmp/static-return-loop-check/static-return-loop-1780524594008/receipt.json` records the source-first static contract, `noscript` fallback, and Manual Copy transport copy; the controlled Chrome gate now also verifies a 320px source-only mirror with `Read source on this device`, `_blank` plus `noreferrer noopener`, a visible `Then capture in Inbox.` secondary action, no horizontal overflow, and a matching source-only `inbox.html` whose Topic hint names the inherited source, is connected by `aria-describedby`, flips to override copy after Source input, and still writes the inherited source into the return patch. Mira returned `PASS_WITH_NOTES`; accepted fixes added unsafe-source, Resume-source, source+open-question ordering, tappable secondary Inbox-link coverage, source links that preserve the mirror page, shorter source-context copy, live override copy, and the "come back to this mirror tab" reminder. Seed's latest cross-device UX fanout only had one successful worker (`phone-harmony-return-critique`); two workers hit max turns and are not treated as evidence.
- The morning review pack now generates `DOGFOOD_RUNBOOK.md` and links it as the first `review-start-here.html` inspection card. It is deliberately `EVIDENCE: PENDING_USER_GATE`, with 11 `NT` rows for real Mac study-loop and manual device-loop dogfood; fixture receipts cannot fill it. Mira reviewed the first slice as `PASS_WITH_NOTES`; accepted follow-up fixes added time budgets, required friction fields, a BLOCKED-needs-reason validator, manual transport recording, `scripts/serve-morning-demo.mjs` for an HTTP dashboard path, `scripts/return-file-import-dry-run.mjs` for replaying selected return JSON through real model import functions without writing the updated workspace, and `scripts/validate-dogfood-runbook.mjs` for turning a filled runbook into a machine-readable receipt. This slice passed `node --check scripts/build-morning-demo.mjs`, `node --check scripts/validate-morning-receipts.mjs`, `node --check scripts/serve-morning-demo.mjs`, `node --check scripts/return-file-import-dry-run.mjs`, `node --check scripts/validate-dogfood-runbook.mjs`, `git diff --check`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning`, `npm run morning:receipts`, `npm run demo:morning:serve:smoke`, `npm run demo:return-import-dry-run:smoke`, `npm run dogfood:validate:smoke`, and `MORNING_DEMO_SKIP_CLEAN=1 LC_KEEP_CHECK_ARTIFACTS=1 npm run check:morning`. Latest static-return receipt: `.codex-tmp/static-return-loop-check/static-return-loop-1780543358567/receipt.json`; dry-run receipt: `.codex-tmp/return-import-dry-run/receipt.json`; dogfood validator receipt: `.codex-tmp/dogfood-runbook/receipt.json`. The validator receipt currently has `pass=0`, `nt=11`, `canClaimMacDogfoodUsable=false`, and `canClaimManualDeviceLoopUsable=false`, as expected before a real run. In-app Browser refused the generated `file://` dashboard under its URL policy, so use `npm run demo:morning:serve -- --port 5174` and open `http://127.0.0.1:5174/` for the dashboard.
...
- Static mirror `index.html` now treats source-only phone/Windows exports as a reading continuation instead of a blank capture prompt: with no due cards or open questions, a safe source link becomes `Read source on this device` or `Resume source on this device`, opens in a new tab/window so the mirror page stays available, tells the user to come back to this mirror tab for return JSON, while `Then capture in Inbox.` remains the tappable return-file step. The matching static Inbox now shows the selected topic source below Topic, says new captures use it unless Source/URL is filled below, flips to override copy when Source or URL is typed, and keeps the inherited source in the generated return patch when the override fields stay empty. Static pages also include a `noscript` fallback that says the mirror remains readable but return-file creation needs JavaScript. After Copy/Save, Review and Inbox now lead with phone-specific transport steps: use Copy or Manual Copy, paste the JSON into a note/email/message, send it to Mac, then import or paste from Today > Return Files; AirDrop/USB/file share/manual Feishu Drive remain optional carriers after a JSON file exists. Due review and open questions still outrank source reading; unsafe or missing source links fall back to `Capture on this device`.
- Mac-side Device Flow and mirror-export receipts now tell the user to open `index.html` first on phone or Windows and follow `Next from this export`, keeping the Mac handoff copy aligned with the static mirror's due/question/source recommendation.
- The generated morning review dashboard now starts with a Dogfood Route card that points to `DOGFOOD_RUNBOOK.md`. The runbook forces real review rows to stay `NT` until Tony records actual step count, elapsed time, failures, friction notes, manual transport, and PASS/FAIL/BLOCKED outcomes for the Mac study loop and any phone/Windows manual device loop. `BLOCKED` rows must carry a reason in the friction notes cell.
- `npm run demo:return-import-dry-run -- --workspace <workspace-or-mirror.json> --return-file <return.json> --out .codex-tmp/return-import-dry-run/<name>.json` can now replay a real returned JSON through the same model import functions used by Mac. It is a dry-run helper only: it reports counts and wrong-file errors, sets `outputWorkspaceWritten=false`, and cannot fill dogfood rows unless the real file movement and Mac UI path were also executed.
- `npm run dogfood:validate -- --runbook dist/morning-demo/DOGFOOD_RUNBOOK.md --out .codex-tmp/dogfood-runbook/real-run-receipt.json` validates a filled runbook. It only allows a Mac dogfood claim when all Mac Study Loop rows are executed without FAIL/BLOCKED and Mac-loop friction is filled; it only allows a manual device-loop claim when all Manual Device Loop rows are executed without FAIL/BLOCKED, transport is filled, and device-loop friction is filled.
...
- Mac manual QA validator review did not return a valid external verdict. Mira failed with `AUTH_EXPIRED`, request id `0ce255d5-ab10-4a4c-b8dc-263413357492`; Seed reached max turns (14); Coco was unavailable first by DNS and then by `401 Unauthorized: invalid token`. This slice therefore proceeded as a conservative local claim-narrowing validator, not as externally approved design work. The unblock path is to refresh Mira/Coco auth or rerun a narrower Seed review before expanding the Mac QA gate beyond validation.
- Mira did not return a verdict for the dogfood Recall counter packet (`dogfood-recall-source-return-counters-20260604`): broker status was `ok=false`, `error_code=AUTH_EXPIRED`, `error_stage=provider`, request id `31d16482-9c0e-46e3-a962-16bba5882737`. Seed (`ark/seed-code-0602`) then reviewed the same evidence contract and returned `VERDICT: PASS_WITH_NOTES`. Accepted: add parallel Save-for-recall source-return and Review-card counters, require them for a completed Mac loop, update Mac Study Loop step 4, define a repeated-Review-card friction signal, and pin the fields in `morning:receipts`. Deferred: actual real dogfood; this only improves the future evidence surface.
- Mira returned `PASS_WITH_NOTES` for the source-missing Quick Capture flow packet (`source-missing-capture-flow-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.78. Accepted: keep source anchoring explicit, document the two-intent split, and keep post-click Source URL focus coverage. Rejected: routing every no-source `focusQuickCapture()` caller to Source URL, because explicit rail/shortcut Quick Capture entry is the intended loose-thought path. Evidence caveat remains: no real Mac dogfood, and in-app Browser only proved DOM/layout because the native bridge was not visible there.
- Mira returned `PASS_WITH_NOTES` for the Recall-card after-save source-first decision packet (`recall-after-save-source-first-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, request id `695d015b-df57-42ea-a5ee-4c260aad9428`, model `re-o-47`, mode `deep`. Accepted: source-linked Recall after-save should become source-first, `reviewCardSourceResume` should be distinct from capture `sourceResume`, direct save and reading/capture promotion should align, Today question-management promotion can remain Review-first, unsafe-source coverage must include promotion, and claims must stay controlled-evidence-only. Deferred: telemetry, real-user evidence that source-return is dominant, Safari/Windows/HarmonyOS/screen-reader checks, and human comprehension.
- Mira returned `PASS_WITH_NOTES` for the static return-location hint packet (`static-return-location-hints-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.78. Accepted: Downloads/Files wording needed to be conditional, Feishu needed to be labeled as manual carrier only, an other-device browser download fallback was useful, and the static-return assertion should not pin one full sentence. Deferred: real Windows/Harmony browser visual QA, narrow phone screenshot evidence, and a real manual device loop.
- Mira returned `PASS_WITH_NOTES` for the after-card next-step packet (`after-card-made-review-next-20260604`) and the follow-up source-resume packet (`after-card-made-source-resume-followup-20260604`), both with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`. Accepted at that historical point: removing `Export Mirror` from the immediate after-card path was correct, the behavior change was real rather than copy-only, `while the point is fresh` was too forceful, and source-linked cards should default the hint to `Resume source` while keeping `Review` visible as the Activity main action. Superseded by the later `recall-after-save-source-first-20260604` review: reading/capture Recall-card saves are now source-first primary with `Review card` secondary. Remaining deferred items: telemetry, real dogfood, Safari/manual visual QA, Windows/HarmonyOS behavior, screen-reader audit, and human comprehension.
- Mira returned `PASS_WITH_NOTES` for the source-resume text-fragment packet (`source-resume-text-fragment-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.78. Accepted: quote sanitizer needed edge coverage, linked/generated answer quote should not anchor source text, `.pdf` links should not get text fragments, model output should expose whether a fragment was generated, explicit capture override needed coverage, and URL-string evidence needed a real Chromium text-fragment smoke. Implemented follow-up: `smoke:text-fragment` plus sanitizer/override tests. Not run: Safari fallback/manual visual QA, real Windows/HarmonyOS browser behavior, fragment failure telemetry, and real dogfood.
- Mira returned `PASS_WITH_NOTES` for the `Open at quote` hint packet (`open-at-quote-hint-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.78. Accepted: label separation is directionally correct, `Open at quote` is acceptable if aria weakens the support promise, review-card hint should be shorter, and smoke should include a no-fragment negative case. Implemented follow-up: aria labels now say `jump ... if supported`, review-card text is shorter, and `smoke:source-resume` verifies a source-backed thought-only capture remains `afterCaptureSavedSourceLinked` / `Open source` / bare URL. Deferred: `textFragmentConfidence`, fragment miss telemetry/toast, Safari/iOS WebView evidence, and review-queue no-fragment-specific fixture.
- Mira returned `PASS_WITH_NOTES` for the Notes/Recall decision guide packet (`notes-recall-decision-guide-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.78. Accepted: the guide is useful in detail but too noisy in Recent Stack, `connect ideas/remember later` is clearer than `synthesize/practice later`, and `Save for recall` aria/title should avoid implementation jargon like `spaced`. Implemented follow-up: guide is detail-only, copy is `Notes: connect ideas · Recall: remember later`, and recall title/aria says `Save this capture to recall later`. Deferred: learn-once dismissal, sidecar narrow visual screenshot, real dogfood evidence, i18n/long-copy coverage, and explicit unit tests for every future capture type.
- Mira returned `PASS_WITH_NOTES` for the Today entry-flow tightening packet (`today-entry-flow-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.70. Accepted: the First Note + Other devices card still had too much cognitive load, pixel thresholds were too brittle as sole evidence, and the next highest-value step remains real Mac dogfood. Implemented follow-up: collapse Other devices by default, loosen the device-route height threshold to a less brittle compactness bound, and keep full manual route available after expansion. Partially rejected: Mira suggested explicit Enter/Space handling is a smell unless the elements are real buttons; these Learning Flow actions are real `<button type="button">` elements, so the remaining follow-up is to add/keep element-type evidence rather than remove the path blindly. Deferred: screen-reader audit, 125% zoom, and real 15-minute dogfood.
- Mira returned `PASS_WITH_NOTES` for the first-capture loop decision packet (`first-capture-loop-decision-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.70. Accepted: the predicate needed an observed transition back to `Clear`, and the wording should not sound like a rigid gate. Implemented follow-up: `Add to notes` now proves the loop clears in smoke, and the loop detail says to choose whether the latest capture belongs in Notes or Review before moving on. Rejected: Mira said capture-decision outranks parked questions, but the current priority order is due review, open question, draft, parked question, then capture decision.
- Mira returned `PASS_WITH_NOTES` for the first-run flow priority packet (`first-run-flow-priority-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.72. Accepted: copy should keep the phone/Windows boundary visible, and support capture negative coverage was too narrow. Implemented follow-up: the loop preview now says `later phone/Windows pass`, and browser smoke covers quote-only plus unlinked-answer captures staying clear without decision styling. Rejected: Mira claimed parked-vs-capture smoke did not assert marker retention, but `latestCaptureNeedsDecision === true` is already pinned in `scripts/smoke-browser.mjs`. Deferred: real Mac dogfood, real narrow-display QA, and first-run comprehension evidence.
- Mira returned `PASS_WITH_NOTES` for the sidecar agent study-loop packet (`sidecar-agent-study-loop-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.72. Accepted: keep it as a smoke/local diagnostic rather than a morning gate, add caveats to the receipt, prove the rail focuses the input, prove `Capture first point` flips to `Capture next point`, and add an adversarial long-text support capture branch. Implemented follow-up in `e67b4f4`. Partially accepted: Mira asked to rename the script/schema to `*-smoke`; the schema/evidence type/output path/npm alias now use smoke wording, but the physical script file was not renamed tonight to avoid delete/rename churn under the no-delete rule. Deferred: WKWebView shell coverage, real human dogfood, and failure screenshots.
- Mira returned `PASS_WITH_NOTES` for the morning-pack controlled sidecar smoke integration (`agent-study-loop-morning-pack-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.78. Accepted: the dashboard row needed the not-dogfood/no-coverage qualifier inline, and labels should converge on one canonical token. Implemented follow-up: `controlled-agent-browser-smoke` is now the dashboard/artifact/Stage label, and `npm run morning:receipts` asserts `not real dogfood` plus `no Mac/Windows/HarmonyOS/Feishu/native picker/file movement coverage`. Rejected/deferred: a clean rebuild without `MORNING_DEMO_SKIP_CLEAN=1` would perform cleanup tonight, and requiring three consecutive smoke runs would make a fast regression receipt too heavy before human dogfood.
- Workflow/Seed user-flow review used `/Users/bytedance/.codex/skills/workflow/scripts/seed_batch.py`, model `ark/seed-code-0602`, two read-only tasks, concurrency 2. `manual-return-flow-critique` succeeded and identified pre-export Device Flow mental-model gaps around ZIP extraction, download fallback location, return-file naming, and Harmony return-patch asymmetry; those were accepted into `6b78078`. `mac-study-loop-critique` returned `Error: Reached max turns (16)` and is not treated as evidence.
- Mira broad user-flow continuity review timed out at the Hermes SSH broker (`error_code=TIMEOUT`, `error_stage=ssh`, elapsed about 630s), so it is not a verdict. A narrower Mira review for the first-run loop preview plus Device Flow guide returned `PASS_WITH_NOTES`, `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.72. Accepted immediately: pending text token, split Harmony patch-asymmetry copy, explicit return-file filename patterns, and narrow-width Device Flow guide browser smoke. Deferred: real 15-minute fresh-profile dogfood, real HarmonyOS/Windows round trips, WCAG contrast/screen-reader audit, and retrying the broad review when the broker path is stable.
...
- Mira returned `PASS_WITH_NOTES` for the static mirror source-first device route. Accepted: source-only mirrors should prioritize reading before capture; unsafe-source, Resume-source, source+open-question ordering, and tappable secondary Inbox-link coverage. Deferred: real HarmonyOS/Windows/iPhone visual QA.
- Mira returned `PASS_WITH_NOTES` for static Inbox topic source context. Accepted: shorter source hint wording, live Source/URL override hint, Topic `aria-describedby`, and browser-smoke assertions for topic title, override copy, and inherited source in the return patch. Deferred: real device, RTL, and large-font QA.
...
- The capture pane now has `Question`, `Answer`, and `Takeaway` starter buttons under a visible `Write as` label. They only seed/convert the local Thought draft prefix and focus the field; Capture/Card still decide what gets committed.
...
- Bilingual rollout plan now lives at `docs/bilingual-i18n-plan.md`. First implementation coverage is intentionally narrow: persisted language preference, visible English/Chinese selector, Learning Flow, and Quick Capture context/guidance/starter copy. Static mirrors, generated receipts, native shell copy, Device Flow, Today full copy, and QA runbooks remain next-slice work unless verified separately.
