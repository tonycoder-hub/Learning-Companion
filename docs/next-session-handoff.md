...
- The morning review pack now generates `DOGFOOD_RUNBOOK.md` and links it as the first `review-start-here.html` inspection card. It is deliberately `EVIDENCE: PENDING_USER_GATE`, with 11 `NT` rows for real Mac study-loop and manual device-loop dogfood; fixture receipts cannot fill it. This slice passed `node --check scripts/build-morning-demo.mjs`, `node --check scripts/validate-morning-receipts.mjs`, `git diff --check`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning`, `npm run morning:receipts`, and `MORNING_DEMO_SKIP_CLEAN=1 LC_KEEP_CHECK_ARTIFACTS=1 npm run check:morning`. Latest static-return receipt: `.codex-tmp/static-return-loop-check/static-return-loop-1780525163072/receipt.json`. In-app Browser refused the generated `file://` dashboard under its URL policy, so no visual Browser pass is claimed for the dashboard.
...
- The generated morning review dashboard now starts with a Dogfood Route card that points to `DOGFOOD_RUNBOOK.md`. The runbook forces real review rows to stay `NT` until Tony records actual step count, elapsed time, failures, and PASS/FAIL/BLOCKED outcomes for the Mac study loop and any phone/Windows manual device loop.
