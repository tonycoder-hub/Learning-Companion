...
Service-worker update visibility: the web shell now surfaces an `App update ready` notice with a manual `Reload` action when an already-controlled page detects a newer service worker. This absorbs Mira's stale-cache risk note without adding automatic refresh, cache deletion, or background sync. Browser verification exposed a real stale-HTML/new-JS mismatch, so `app.js` now installs compatibility-only shell nodes for the update notice and sidecar rail before first render.
