...
- For `97c230a`, `node --check scripts/build-morning-demo.mjs`, `node --check scripts/validate-morning-receipts.mjs`, `git diff --check`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning`, `npm run smoke`, `npm run morning:receipts`, `npm run dogfood:validate:smoke`, and `npm run demo:morning:serve:smoke` passed. Generated `dist/morning-demo/review-start-here.html` contains the new `Morning Dogfood Gate`. A fixed `npm run demo:morning:serve -- --port 5174` check hit local port-binding sandbox `EPERM`; no approval was requested, and the random-port serve smoke remains the executed HTTP proof.
