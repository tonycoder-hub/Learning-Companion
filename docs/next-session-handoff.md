...
- The morning review pack now generates `DOGFOOD_RUNBOOK.md` and links it as the first `review-start-here.html` inspection card. It is deliberately `EVIDENCE: PENDING_USER_GATE`, with 11 `NT` rows for real Mac study-loop and manual device-loop dogfood; fixture receipts cannot fill it. Mira reviewed the first slice as `PASS_WITH_NOTES`; accepted follow-up fixes added time budgets, required friction fields, a BLOCKED-needs-reason validator, manual transport recording, `scripts/serve-morning-demo.mjs` for an HTTP dashboard path, and `scripts/return-file-import-dry-run.mjs` for replaying selected return JSON through real model import functions without writing the updated workspace. This slice passed `node --check scripts/build-morning-demo.mjs`, `node --check scripts/validate-morning-receipts.mjs`, `node --check scripts/serve-morning-demo.mjs`, `node --check scripts/return-file-import-dry-run.mjs`, `git diff --check`, `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning`, `npm run morning:receipts`, `npm run demo:morning:serve:smoke`, `npm run demo:return-import-dry-run:smoke`, and `MORNING_DEMO_SKIP_CLEAN=1 LC_KEEP_CHECK_ARTIFACTS=1 npm run check:morning`. Latest static-return receipt: `.codex-tmp/static-return-loop-check/static-return-loop-1780543087990/receipt.json`; dry-run receipt: `.codex-tmp/return-import-dry-run/receipt.json`. In-app Browser refused the generated `file://` dashboard under its URL policy, so use `npm run demo:morning:serve -- --port 5174` and open `http://127.0.0.1:5174/` for the dashboard.
