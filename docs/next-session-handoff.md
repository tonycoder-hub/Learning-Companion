...
- Adding a capture to Notes now keeps the reading loop alive instead of leaving Notes as a dead end. New or updated note blocks keep Activity's main action as `View note`, but source-linked captures also get a next hint: source-only notes show `Open source`, text-fragment notes show `Open at quote`, and timed/video notes show `Resume source`. If the user only opens an already-current note block, no new source hint is emitted. Resuming from a note now says `The note is saved. Keep reading; capture the next point when it lands.` and returns to Quick Capture while keeping `View note` as the Activity main action. Browser smoke covers new note -> `#:~:text=...`, timed note -> `t=201s`, and stale `Update note` -> `Note updated.` hint text. Mira returned `PASS_WITH_NOTES` twice; accepted follow-up neutralized the resume copy, distinguished update text, added timed-source coverage, boolean-normalized `didUpdate`, and hardened dynamic hint overrides with kind validation plus text/action/aria length limits. Rejected for now: switching new-note main action away from `View note`, because the current Activity pattern keeps the freshly created artifact inspectable and puts source return in the next hint. Dogfood exit condition: during the next 60-90 minute real Mac study run, if three or more `Add to notes` moments immediately seek source return and do not use `View note`, switch new-note main action to source return and demote `View note`.
- The morning dogfood runbook now captures that same Notes/source-return decision quantitatively instead of leaving it as an open opinion. The session header includes `Add-to-Notes source-return count` and `Add-to-Notes View-note count`; Mac Study Loop step 5 now asks the reviewer to insert a capture into Notes, view the generated block, then decide whether to use the source-return hint; and the decision rules say that three or more immediate source-return preferences should be recorded as Mac loop friction and used to demote `View note` in the next implementation pass. The dogfood validator requires those counts to be non-negative integers once Mac rows are actually executed, while generated `NT` runbooks still stay `PENDING_USER_GATE`. Verified with `MORNING_DEMO_SKIP_CLEAN=1 npm run demo:morning`, `npm run dogfood:validate:smoke`, and `npm run morning:receipts`.
