...
- Quick Capture heading now makes the ordinary capture path more dominant. Empty `No draft` status is hidden until a real local draft exists, `Card` and `Cloze` are lighter secondary actions with recall-oriented title/aria copy, and the starter row label changed from `Start draft` to `Write as`. In-app Browser visual QA showed `Capture`, `Card`, and `Cloze` stay on one row at the current sidecar-ish width, `Write as` is visible, and there is no horizontal overflow.
