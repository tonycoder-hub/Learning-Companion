...
- For `8d6b412` plus the follow-up source-only mobile browser coverage, `node --check apps/companion-web/src/model.js`, `node --check scripts/smoke-web.mjs`, `node --check scripts/static-return-loop-check.mjs`, `node --check scripts/smoke-browser.mjs`, `git diff --check`, `npm run smoke`, `npm run check:static-return`, and `npm run smoke:browser` passed. The latest static-return receipt at `.codex-tmp/static-return-loop-check/static-return-loop-1780522725669/receipt.json` records `sourceFirstDeviceRoute=true` and `sourceOpensInNewTab=true`; the controlled Chrome gate now also verifies a 320px source-only mirror with `Read source on this device`, `_blank` plus `noreferrer noopener`, a visible `Then capture in Inbox.` secondary action, and no horizontal overflow. Mira returned `PASS_WITH_NOTES`; accepted fixes added unsafe-source, Resume-source, source+open-question ordering, tappable secondary Inbox-link coverage, and source links that preserve the mirror page.
...
- Static mirror `index.html` now treats source-only phone/Windows exports as a reading continuation instead of a blank capture prompt: with no due cards or open questions, a safe source link becomes `Read source on this device` or `Resume source on this device`, opens in a new tab/window so the mirror page stays available, while `Then capture in Inbox.` remains the tappable return-file step. Due review and open questions still outrank source reading; unsafe or missing source links fall back to `Capture on this device`.
