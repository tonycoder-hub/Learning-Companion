...
- Coverage boundary for that loop: it proves the local static HTML/HTTP harness contract and Mac import resolver compatibility. It still does not prove real `file://` behavior, HarmonyOS/Windows browser storage/download quirks, Feishu Drive transport, or multi-answer batch return files.
