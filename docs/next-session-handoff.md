...
- For `8d6b412`, `node --check apps/companion-web/src/model.js`, `node --check scripts/smoke-web.mjs`, `node --check scripts/static-return-loop-check.mjs`, `git diff --check`, `npm run smoke`, and `npm run check:static-return` passed. The latest static-return receipt at `.codex-tmp/static-return-loop-check/static-return-loop-1780522725669/receipt.json` records `sourceFirstDeviceRoute=true` and `sourceOpensInNewTab=true`. Mira returned `PASS_WITH_NOTES`; accepted fixes added unsafe-source, Resume-source, source+open-question ordering, tappable secondary Inbox-link coverage, and source links that preserve the mirror page.
