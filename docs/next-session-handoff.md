...
- Static Inbox now has a storage-failure path: if `localStorage` get/set fails, the page keeps the current return draft in memory, keeps `patchPreview` usable, and tells the user to use Manual Copy or Save before closing. Browser smoke blocks `localStorage` before loading `inbox.html` and verifies Add Capture still produces a valid return patch; it also forces clipboard and save-picker fallback failure so the preview is selected for manual copy. Follow-up: apply the same storage hardening to static `review.html` after this Inbox loop is committed.
