...
- For `8d6b412`, `node --check apps/companion-web/src/model.js`, `node --check scripts/smoke-web.mjs`, `node --check scripts/static-return-loop-check.mjs`, `git diff --check`, `npm run smoke`, and `npm run check:static-return` passed. The static-return receipt at `.codex-tmp/static-return-loop-check/static-return-loop-1780522575028/receipt.json` records `sourceFirstDeviceRoute=true`. Mira returned `PASS_WITH_NOTES`; accepted fixes added unsafe-source, Resume-source, source+open-question ordering, and tappable secondary Inbox-link coverage.
...
- Static mirror `index.html` now treats source-only phone/Windows exports as a reading continuation instead of a blank capture prompt: with no due cards or open questions, a safe source link becomes `Read source on this device` or `Resume source on this device`, while `Then capture in Inbox.` remains the tappable return-file step. Due review and open questions still outrank source reading; unsafe or missing source links fall back to `Capture on this device`.
