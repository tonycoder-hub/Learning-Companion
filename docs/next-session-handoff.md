...
- For `8d6b412`, `node --check apps/companion-web/src/model.js`, `node --check scripts/smoke-web.mjs`, `git diff --check`, and `npm run smoke` passed. Mira returned `PASS_WITH_NOTES`; accepted fixes added unsafe-source, Resume-source, source+open-question ordering, and tappable secondary Inbox-link coverage.
