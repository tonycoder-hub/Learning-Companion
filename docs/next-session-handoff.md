...
- After a capture/highlight is saved for recall, Activity no longer suggests `Export Mirror` as the next hint. `afterCardMade` now says `Saved for recall. Review when you want, or keep reading.`, uses `Review card`, and opens the new review card by card id through a shared `openReviewCard()` helper. The hint now hides if the target review card no longer exists. Browser smoke proves the hint opens Review, sets focus mode to `review`, activates the generated card, and does not route to Device Flow. Mira returned `PASS_WITH_NOTES`; accepted follow-up softened the "while fresh" wording. Deferred: a larger dual-action `Resume source`/`Review card` model, telemetry, cross-session edge coverage, and real dogfood data.
...
- For the after-card next-step slice, `node --check apps/companion-web/src/app.js`, `node --check scripts/smoke-browser.mjs`, `node --check scripts/smoke-web.mjs`, `git diff --check`, `npm run smoke`, and `npm run smoke:browser` passed. `npm run smoke` returned `smoke_web_ok`; `npm run smoke:browser` returned `smoke_browser_ok`. The first browser-smoke attempt failed because the expected prompt was incorrectly assumed to be the quote; the real review prompt came from the added thought (`Recall the point behind: ...`), and the test was corrected without changing product logic.
...
- Mira returned `PASS_WITH_NOTES` for the after-card next-step packet (`after-card-made-review-next-20260604`), with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`, confidence 0.72. Accepted: removing `Export Mirror` from the immediate after-card path is correct, the behavior change is real rather than copy-only, and the initial `while the point is fresh` wording was too forceful. Implemented follow-up: softer `Saved for recall. Review when you want, or keep reading.` copy. Deferred: a dual-action `Resume source` primary / `Review card` secondary model, hint telemetry, cross-session/card-deleted click coverage, and real dogfood.
