...
- Plain source-backed quote+thought captures now show a source next-step hint after save, matching the question and highlight loops without overclaiming playback state: bare source URLs show `Open source`, timestamped captures show `Resume source`. The existing full browser smoke now asserts the hint appears, and `npm run smoke:source-resume` verifies the focused browser paths: bare source open, source-less capture negative case, timestamped YouTube resume with `t=42s`, separate tab target with `noopener,noreferrer`, return to Quick Capture, and focus the thought field for the next point.
