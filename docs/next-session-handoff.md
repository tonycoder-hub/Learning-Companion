...
- Browser smoke now covers the missing mirror-home answer click-through. It generates a dedicated answer mirror by creating a real open-question capture through the UI, verifies `index.html` links into `inbox.html?answerToCaptureId=...`, lands in read-only `Question from Mac` / writable `Answer to return` mode, proves the answer textarea accepts input, and asserts the baseline workspace was restored so the main smoke flow keeps its original no-open-question state.
