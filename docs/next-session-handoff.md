...
- Post-save source-return hint readability now has a focused visual evidence path. `npm run smoke:post-save-hints` drives a credential-free headless Chrome app instance through source-backed Add-to-Notes and Save-for-recall states at 390x760 and 1280x720, writes a receipt to `.codex-tmp/post-save-hint-visual/receipt.json`, and stores activity-strip screenshots under `.codex-tmp/post-save-hint-visual/run-*/screenshots/`. The receipt schema is `learning-companion.post-save-hint-visual.v1`, evidence type is `CONTROLLED_HEADLESS_BROWSER_VISUAL`, and `provesRealUserDogfood=false`; it checks no horizontal overflow, visible hint text/action, no hint/button overlap, and no Activity action overflow. It does not prove Safari, Windows, HarmonyOS, screen-reader behavior, or human comprehension.
- Source-linked recall card cross-session behavior now has explicit controlled browser coverage. The post-save smoke verifies a source-linked `afterCardMadeTextSourceLinked` review-card hint is visible in the source session, switches to a newly created session where the hint is hidden, force-clicks the stale hidden hint button and confirms no source opens, then restores the source session and confirms the hint returns. Product behavior was unchanged; this closes the prior cross-session after-card hint evidence gap without claiming real dogfood.
...
- Source-linked recall card deletion now has explicit controlled browser coverage. The post-save smoke creates a source-linked highlight card with an `Open at quote` hint, removes the backing review card from the fixture workspace before the hint click, verifies the Activity hint hides, then force-clicks the hidden hint button and confirms `Review card no longer exists` is shown with no `window.open` call. Restoring the card makes the hint available again and the original source-resume path still works. This is fail-safe coverage for a card-deleted race; product behavior was unchanged.
- Recall-card source fallback now has explicit controlled browser coverage for unsafe source URLs. The post-save smoke creates a capture/card after entering `javascript:alert(1)` as the source URL and verifies the saved session/capture source URLs are blank, the review card still exists, Activity falls back to `afterCardMade` with `Review card`, clicking the hint opens the review card, and `window.open` is not called. This closes the prior source-invalid fallback gap for the Recall after-save path without changing product behavior or claiming real browser/device dogfood.
...
- After a capture/highlight is saved for recall, Activity no longer suggests `Export Mirror` as the next hint. The Activity main action remains `Review`, but source-linked cards now use an `afterCardMadeSourceLinked` hint: `Saved for recall. Jump back to the source; the card is here when you want to review.` with `Resume source` as the hint action. No-source card creation still offers `Review card`. The hint uses the review card -> source capture -> source URL chain and hides if the target card/source is unavailable. Browser smoke proves the source-linked hint opens the source in a separate tab, returns the app to Captures, focuses `thoughtInput`, pulses Quick Capture, and keeps the review card accessible via the main Activity action. Mira returned `PASS_WITH_NOTES`; accepted follow-up made resume source the default source-linked hint and softened Review into a secondary option. Follow-up completed: ordinary article/document captures now use Chromium text fragments when there is no valid timestamp and the capture quote can safely anchor the source. Deferred: telemetry, cross-session edge coverage, source-invalid fallback smoke, Safari/manual visual QA, and real dogfood data.
...
- For the source-linked recall card deleted-hint smoke slice, `node --check scripts/smoke-browser.mjs`, `git diff --check`, and `npm run smoke:browser` passed. The first `npm run smoke:browser` attempt failed in an older import-negative fixture because `malformedImportReceipt` still showed the previous `bad-mirror.json` receipt instead of `broken-patch.json`; rerunning the same command returned `smoke_browser_ok`, so this is recorded as an existing receipt timing flake rather than evidence against the new deleted-card path. The new branch proves card removal hides the source hint, a forced hidden-button click does not call `window.open`, and restoring the card restores `afterCardMadeTextSourceLinked`.
- For the Recall unsafe-source fallback smoke slice, `node --check scripts/smoke-browser.mjs`, `git diff --check`, and `npm run smoke:browser` passed. `npm run smoke:browser` returned `smoke_browser_ok`. The new controlled browser branch verifies an unsafe source URL is sanitized to blank for the session and capture, the card remains reviewable, Activity uses `afterCardMade` / `Review card`, and the hint click opens the review card without invoking `window.open`. This is controlled Chrome smoke evidence only; real Safari/Windows/HarmonyOS/manual dogfood remains Not Run.
...
- Mira returned `PASS_WITH_NOTES` for the after-card next-step packet (`after-card-made-review-next-20260604`) and the follow-up source-resume packet (`after-card-made-source-resume-followup-20260604`), both with `cleanup_succeeded=true`, `logid_present=true`, model `re-o-47`, mode `deep`. Accepted: removing `Export Mirror` from the immediate after-card path is correct, the behavior change is real rather than copy-only, `while the point is fresh` was too forceful, and source-linked cards should default the hint to `Resume source` while keeping `Review` visible as the Activity main action. Implemented follow-up: `afterCardMadeSourceLinked` plus low-pressure copy. Deferred: scroll anchor/highlight focus, hint telemetry, cross-session/card-deleted click coverage, source-invalid fallback smoke, and real dogfood.
