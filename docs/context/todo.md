...
- `scripts/next-major-operator-packet.mjs`: writes a non-claiming `learning-companion.next-major-operator-packet.v1` JSON/Markdown operator packet with `NEXT_MAJOR_OPERATOR_PACKET_ONLY` and `releaseActionAuthorized: false`. It can refresh the local KO status, readiness packet, and platform handoff, then combines them with the current source approval request into one lane-ordered execution packet. It refuses to treat a stale or dirty public dry-run-based source approval request as ready for current-turn approval and instead marks `NEEDS_FRESH_PUBLIC_DRY_RUN_OR_APPROVAL_REQUEST`. It does not grant current-turn source approval, run browser evidence, perform privacy review, run Mac/Windows/HarmonyOS QA, build, package, deploy, or satisfy KO evidence. `npm run smoke` includes a fixture CLI run to verify the operator packet remains non-claiming, detects stale public dry-run approval requests, and writes `0600` outputs.
