...
- `scripts/validate-external-source-privacy-review.mjs`: generates a human privacy-review template and validates completed reviews into a derived `APPROVED_SOURCE_PRIVACY_REVIEWED` artifact only for real approved-source candidate receipts. It refuses local fixture self-tests, privacy-review self-test artifact paths, and requires the screenshot review list to exactly cover the candidate files with `PASS`, matching `bytes`, matching `sha256`, source approval confirmation, source-context preservation, video timestamp evidence, approved video timestamp review binding, non-placeholder reviewer / approval reference / notes, and an ISO `reviewedAt` date-time with timezone; screenshot files must exist and be non-empty. The derived KO artifact carries the reviewed screenshot file list plus `bytes` / `sha256`, and the top-level KO gate now rereads the referenced privacy-review artifact to verify its schema, receipt path, `PASS` verdict, KO-use flag, source-approval fields, privacy/execution review booleans, reviewer/reviewedAt, and reviewed screenshot `bytes` / `sha256`; it also recomputes non-empty file values and requires the reading/video screenshot filename matrix including `02b-video-learning-tools.png` and `04-video-timestamp.png` before accepting the external-source claim. Its self-test writes a fixture-only summary with `canClaimExternalKo: false`, not a claim-shaped KO artifact.
...
- `scripts/next-major-operator-packet.mjs`: writes a non-claiming `learning-companion.next-major-operator-packet.v1` JSON/Markdown operator packet with `NEXT_MAJOR_OPERATOR_PACKET_ONLY` and `releaseActionAuthorized: false`. It can refresh the local KO status, readiness packet, and platform handoff, then combines them with the current source approval request into one lane-ordered execution packet. It refuses to treat a stale or dirty public dry-run-based source approval request as ready for current-turn approval and instead marks `NEEDS_FRESH_PUBLIC_DRY_RUN_OR_APPROVAL_REQUEST`; it also tracks readiness-packet freshness and keeps the final KO lane blocked until the readiness packet was generated from the current clean HEAD. It does not grant current-turn source approval, run browser evidence, perform privacy review, run Mac/Windows/HarmonyOS QA, build, package, deploy, or satisfy KO evidence. `npm run smoke` includes a fixture CLI run to verify the operator packet remains non-claiming, detects stale public dry-run approval requests and stale readiness packets, and writes `0600` outputs.
- `scripts/finalize-next-major.mjs`: after final KO validation, refreshes readiness/platform/operator packets and refuses the finalizer output unless the operator packet remains non-claiming, has current readiness/platform freshness, and its final KO lane is ready to validate final KO. It still does not grant approval, perform privacy review, run platform QA, build, package, deploy, remote-accept, or authorize release.
- `scripts/validate-ko-evidence.mjs`, `scripts/ko-next-action-summary.mjs`, and `scripts/platform-qa-handoff.mjs`: real platform QA receipt paths are now aligned. The handoff writes real-run commands to `.codex-tmp/mac-manual-qa/real-run-receipt.json`, `.codex-tmp/windows-static-qa/real-run-receipt.json`, and `.codex-tmp/harmony-device-qa/real-run-receipt.json` by default, and `--mac-manual`, `--windows-static`, and `--harmony-device` path bindings now flow through readiness, `next:operator -- --refresh`, stale-handoff refresh commands, and `next:finalize` into the regenerated platform handoff and final gate commands; `ko:next -- --refresh` and `ko:validate` auto-read default real-run paths when present, falling back to pending smoke receipts only before real QA exists. For any filled/claimable platform receipt, the KO gate now rereads the referenced platform handoff artifact and verifies that it is the current clean non-claiming handoff and that its platform lane matches the receipt path, QA path, validation command, and expected row count.
- `scripts/next-major-readiness.mjs` and `scripts/next-major-operator-packet.mjs`: refresh entrypoints now preserve supplied `--external`, `--mac-manual`, `--windows-static`, and `--harmony-device` bindings when regenerating KO status, readiness, platform handoff, and operator packet. `scripts/refresh-next-major-local-evidence.mjs` preserves custom platform receipt bindings through its local refresh chain and explicitly rejects `--external`; approved external KO evidence should be bound through readiness, operator, `ko:next`, or `next:finalize`. This remains evidence refresh only; it does not create current-turn source approval, privacy review, or platform QA evidence.
- `scripts/ko-next-action-summary.mjs`: `--json-out` now writes a private `learning-companion.ko-next-action-summary.v1` handoff artifact with the current KO status, missing requirements, source approval text/commands, platform receipt paths, operator next-action sequence, final gate commands, and explicit not-run boundaries. `scripts/refresh-next-major-local-evidence.mjs` refreshes this artifact at `.codex-tmp/ko-next/current.json` by default. This is still `KO_NEXT_ACTION_SUMMARY_ONLY`; it cannot grant source approval, run browser evidence, perform privacy review, run platform QA, or satisfy KO evidence.
...
- Generate the single operator packet with `npm run next:operator -- --refresh --out .codex-tmp/next-major-operator/current.json --markdown-out .codex-tmp/next-major-operator/current.md`; this combines source approval, platform QA, readiness, and final gate lanes for the next human/device execution pass. If the external-source lane says `NEEDS_FRESH_PUBLIC_DRY_RUN_OR_APPROVAL_REQUEST`, refresh the public dry-run on a clean current HEAD and regenerate the approval request before asking for current-turn approval.
