...
- Privacy-review templates/claims, top-level KO status/final JSON, and Mac/Windows/Harmony platform QA receipt JSON outputs are written as `0600` files after creating new parent directories with `0700`; path-valued CLI flags reject bare boolean use so missing paths do not turn into accidental output locations.
