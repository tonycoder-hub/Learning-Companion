...
- Privacy-review templates/claims and top-level KO status/final JSON files are written with explicit `0600` mode after creating new parent directories with `0700`; bare path-valued flags such as `--out` or `--external` are rejected before accidental boolean paths can be used.
...
- Mac/Windows/Harmony platform QA validator receipt JSON outputs are written with explicit `0600` mode after creating new parent directories with `0700`; bare `--qa` and `--out` flags are rejected before the validator reads Markdown or writes a receipt.
