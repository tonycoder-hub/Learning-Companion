...
- Privacy-review templates/claims and top-level KO status/final JSON files are written with explicit `0600` mode after creating parent directories with `0700`; bare path-valued flags such as `--out` or `--external` are rejected before accidental boolean paths can be used.
...
Most recent focused rerun after hardening privacy-review and KO evidence output files passed `node --check scripts/validate-external-source-privacy-review.mjs`, `node --check scripts/validate-ko-evidence.mjs`, `node --check scripts/smoke-web.mjs`, `npm run external:privacy-review:selftest`, `npm run ko:validate:selftest`, wrapped JSON mode checks for the generated self-test summaries, wrapped missing-value negative checks for `npm run external:privacy-review -- --out` and `npm run ko:validate -- --external`, `npm run smoke`, `npm run ko:next -- --refresh`, and `git diff --check`. This only tightens local evidence file permissions and argument handling; KO remains non-claimable until real approved-source and platform QA evidence exist.
