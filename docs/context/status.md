...
- `npm run external:approval-request` now turns either a source-intake handoff or a public dry-run receipt into a non-claiming `learning-companion.external-source-approval-request.v1` JSON/Markdown packet with the exact requested approval text and approved-candidate command. It keeps `evidenceTier: SOURCE_APPROVAL_REQUEST_ONLY`, `canClaimExternalKo: false`, and does not grant approval, launch Chromium, capture screenshots, perform privacy review, or satisfy KO evidence.
- Real video runs now also require `--video-timestamp` so the harness cannot pass a video candidate without timestamp evidence.
- `npm run external:validate:selftest` launches local reading/video fixtures in headless Chromium, captures the source/app evidence screenshots, writes `receipt.json` and `run.md` as private files, and keeps `canClaimExternalKo: false`.
...
- External-source screenshots, run notes, source-intake handoffs, privacy-review templates/claims, and top-level KO status/final JSON files are written with explicit `0600` mode after creating new parent directories with `0700`; existing output files are chmodded to `0600` before overwrite, and bare path-valued flags such as `--out`, `--out-root`, or `--external` are rejected before accidental boolean paths can be used.
- External-source browser runs now shut down Chromium and remove the throwaway browser profile before writing the receipt; `runContext.browser.profileRetained` must be `false`, cleanup must be `ok: true`, and both the privacy-review validator and top-level KO gate reject approved external claims without that cleanup proof. Self-tests now cover retained-profile and cleanup-failed claim rejection.
...
- The latest current-head public-source dry-run is `.codex-tmp/external-source-validation/20260622T210546Z-public-source-dry-run/receipt.json`. It used the same public Wikipedia/MDN preflight sources, recorded git HEAD `efedb399be909c80f1e59216580238e409df3bda`, recorded the currently dirty worktree during this hardening slice, removed the throwaway Chromium profile with `profileCleanup.ok: true`, saved reading/video screenshots and `run.md` as `0600`, kept `canClaimExternalKo: false`, and remained excluded from KO by `npm run ko:next -- --refresh`.
...
- `scripts/platform-qa-handoff.mjs` reads the current KO status plus the generated Mac, Windows, and HarmonyOS QA templates and emits a `learning-companion.platform-qa-handoff.v1` handoff with `PLATFORM_QA_HANDOFF_ONLY`, `canClaimKo: false`, exact validation commands, required session fields, row/result counts, and explicit no-platform-run boundaries. It can also write a human-readable Markdown execution packet for real platform operators. It does not retain raw QA Markdown or row Notes, run platform QA, or satisfy KO evidence.
- The KO gate and `ko:next -- --refresh` now auto-select real platform QA receipts when present (`.codex-tmp/mac-manual-qa/real-run-receipt.json`, `.codex-tmp/windows-static-qa/real-run-receipt.json`, `.codex-tmp/harmony-device-qa/real-run-receipt.json`) and fall back to pending smoke receipts only when real-run receipts do not exist.
...
- Mac/Windows/Harmony platform QA validator receipt JSON outputs are written with explicit `0600` mode after creating new parent directories with `0700`; existing output files are chmodded to `0600` before overwrite, and bare `--qa` and `--out` flags are rejected before the validator reads Markdown or writes a receipt.
...
- `npm run platform:qa-handoff -- --out .codex-tmp/platform-qa-handoff/current.json --markdown-out .codex-tmp/platform-qa-handoff/current.md` -> non-claiming platform execution handoff; summarizes Mac/Windows/Harmony pending receipts without running platform QA and writes a readable operator packet
...
Most recent focused rerun after hardening privacy-review and KO evidence output files passed `node --check scripts/validate-external-source-privacy-review.mjs`, `node --check scripts/validate-ko-evidence.mjs`, `node --check scripts/smoke-web.mjs`, `npm run external:privacy-review:selftest`, `npm run ko:validate:selftest`, wrapped JSON file-mode checks for the generated self-test summaries, wrapped missing-value negative checks for `npm run external:privacy-review -- --out` and `npm run ko:validate -- --external`, `npm run smoke`, `npm run ko:next -- --refresh`, and `git diff --check`. This only tightens local evidence file permissions and argument handling; KO remains non-claimable until real approved-source and platform QA evidence exist.

Most recent focused rerun after hardening platform QA validator receipt outputs passed `node --check scripts/validate-mac-manual-qa.mjs`, `node --check scripts/validate-windows-static-qa.mjs`, `node --check scripts/validate-harmony-device-qa.mjs`, `node --check scripts/smoke-web.mjs`, `npm run smoke`, `npm run mac:manual:validate:smoke`, `npm run windows:static:validate:smoke`, `npm run harmony:device:validate:smoke`, wrapped receipt file-mode checks, wrapped missing-value negative checks for each validator's `--qa` and `--out`, `npm run ko:validate:selftest`, `npm run ko:next -- --refresh`, and `git diff --check`. This keeps platform QA receipts non-claiming until real rows are filled and only hardens receipt output handling.

Most recent focused rerun after closing existing-output permission windows passed `node --check scripts/external-source-validation-browser.mjs`, `node --check scripts/validate-external-source-privacy-review.mjs`, `node --check scripts/validate-ko-evidence.mjs`, `node --check scripts/platform-qa-handoff.mjs`, `node --check scripts/smoke-web.mjs`, `npm run smoke`, `npm run external:validate:selftest`, `npm run external:privacy-review:selftest`, `npm run ko:validate:selftest`, `npm run platform:qa-handoff -- --out .codex-tmp/platform-qa-handoff/current.json`, wrapped existing-file mode checks after pre-setting outputs to `0666`, wrapped missing-value negative checks for source-intake `--out`, external validation `--out-root`, `--video-timestamp`, and note/input flags, `npm run ko:next -- --refresh`, and `git diff --check`. This only hardens local evidence artifacts and leaves KO non-claimable until real approved-source and platform QA evidence exist.

Most recent focused rerun after adding the platform QA Markdown execution packet passed `node --check scripts/platform-qa-handoff.mjs`, `node --check scripts/ko-next-action-summary.mjs`, `node --check scripts/smoke-web.mjs`, `npm run platform:qa-handoff -- --out .codex-tmp/platform-qa-handoff/current.json --markdown-out .codex-tmp/platform-qa-handoff/current.md`, a JSON/Markdown shape and `0600` mode check, `npm run smoke`, `npm run ko:next -- --refresh`, a wrapped `--markdown-out` missing-path negative check, and `git diff --check`. The generated Markdown packet lists missing KO requirements, required session fields, validation commands, non-substitute evidence sources, row areas, final gate commands, and the explicit no-platform-run boundary; it remains `PLATFORM_QA_HANDOFF_ONLY` and cannot satisfy KO evidence.

Most recent focused rerun after adding the next-major readiness packet passed `node --check scripts/next-major-readiness.mjs`, `node --check scripts/ko-next-action-summary.mjs`, `node --check scripts/smoke-web.mjs`, `npm run next:readiness -- --refresh --out .codex-tmp/next-major-readiness/current.json --markdown-out .codex-tmp/next-major-readiness/current.md`, a JSON/Markdown shape and `0600` mode check, `npm run smoke`, `npm run ko:next -- --refresh`, a wrapped `--markdown-out` missing-path negative check, a malformed-status negative check where `canClaimKo: true` with missing requirements is rejected, and `git diff --check`. `npm run smoke` also feeds a claimable KO-status fixture through `scripts/next-major-readiness.mjs` and verifies the packet can report `PRE_RELEASE_EVIDENCE_READY` while `releaseActionAuthorized` stays `false`, packet-level not-run boundaries remain populated, and JSON/Markdown outputs are `0600`. The script refreshes the existing KO status with `--allow-missing`, validates the expected KO status schema and required requirement IDs, then writes a private JSON/Markdown `learning-companion.next-major-readiness.v1` packet with `NEXT_MAJOR_READINESS_SUMMARY_ONLY`, the current pass/fail/missing requirements, platform QA statuses, next commands, and explicit not-run boundaries. It sets `releaseActionAuthorized: false`, keeps packet-level not-run boundaries unconditional, and does not run approved external-source validation, privacy review, platform QA, build, package, deployment, Mew-Test, main-site, remote acceptance checks, or authorize release.

Most recent focused rerun after adding the next-major operator packet passed `node --check scripts/next-major-operator-packet.mjs`, `node --check scripts/ko-next-action-summary.mjs`, `node --check scripts/smoke-web.mjs`, `npm run next:operator -- --refresh --out .codex-tmp/next-major-operator/current.json --markdown-out .codex-tmp/next-major-operator/current.md`, a JSON/Markdown shape and `0600` mode check, `npm run smoke`, `npm run ko:next -- --refresh`, a wrapped `--markdown-out` missing-path negative check, and `git diff --check`. `npm run smoke` also feeds fixture KO/readiness/platform/source-approval inputs through `scripts/next-major-operator-packet.mjs` and verifies the operator packet remains non-claiming, keeps `releaseActionAuthorized: false`, includes source-approval and real-platform-run lanes, lists explicit not-run boundaries, and writes `0600` outputs. The operator packet has `NEXT_MAJOR_OPERATOR_PACKET_ONLY`; it consolidates readiness, source approval request, and platform QA handoff data into one execution packet, but it does not grant current-turn approval, run approved-source browser evidence, perform privacy review, run Mac/Windows/HarmonyOS QA, build, package, deploy, remote-accept, or satisfy KO evidence.

Most recent focused rerun after adding source approval request freshness checks passed `node --check scripts/next-major-operator-packet.mjs`, `node --check scripts/smoke-web.mjs`, `node --check scripts/ko-next-action-summary.mjs`, `npm run next:operator -- --refresh --out .codex-tmp/next-major-operator/current.json --markdown-out .codex-tmp/next-major-operator/current.md`, `npm run smoke`, a JSON/Markdown shape and `0600` mode check, a wrapped `--markdown-out` missing-path negative check, and `git diff --check`. The operator packet now records current git HEAD / dirty state, checks source approval requests that were generated from `PUBLIC_SOURCE_DRY_RUN` receipts, and marks them `NEEDS_FRESH_PUBLIC_DRY_RUN_OR_APPROVAL_REQUEST` when the prior dry-run git HEAD differs from current HEAD, the prior dry-run was dirty, the current worktree is dirty, or browser profile cleanup was not proven. The existing source approval request was correctly downgraded from `NEEDS_CURRENT_TURN_APPROVAL` because it was based on git HEAD `efedb399be909c80f1e59216580238e409df3bda` with `dirtyWorktree: true`, while the current HEAD is newer.
...
1. If `npm run next:operator -- --refresh --out .codex-tmp/next-major-operator/current.json --markdown-out .codex-tmp/next-major-operator/current.md` reports `NEEDS_FRESH_PUBLIC_DRY_RUN_OR_APPROVAL_REQUEST`, refresh the public-source dry-run on a clean current HEAD, regenerate `.codex-tmp/external-source-validation/source-approval-request.json`, then request current-turn approval from that refreshed request.
2. Get exact approved reading and video source URLs plus the intended video timestamp, then run `npm run external:validate -- --approved-current-turn --reading-url <approved-reading-url> --video-url <approved-video-url> --video-timestamp <captured-timestamp> --approval-note "<current-turn approval>"`.
3. Generate and fill the privacy review with `npm run external:privacy-template -- --receipt <candidate-receipt.json> --out <privacy-review.json>`, then validate it with `npm run external:privacy-review -- --receipt <candidate-receipt.json> --review <privacy-review.json> --out <ko-evidence-review.json>`.
4. Generate the non-claiming platform execution handoff with `npm run platform:qa-handoff -- --out .codex-tmp/platform-qa-handoff/current.json --markdown-out .codex-tmp/platform-qa-handoff/current.md`, then fill and validate native Mac manual QA, Windows static/manual QA, and HarmonyOS device/toolchain QA receipts from real runs. The handoff writes real-run receipt commands to `.codex-tmp/*/real-run-receipt.json`, which the KO gate now reads by default when present.
5. Run the top-level gate with `npm run ko:validate -- --external <ko-evidence-review.json> --out .codex-tmp/ko-evidence/final.json` only after the external artifact and all three platform receipts pass. If a custom receipt path is used, pass `--mac-manual`, `--windows-static`, and `--harmony-device` explicitly.
6. Use `npm run next:readiness -- --refresh --out .codex-tmp/next-major-readiness/current.json --markdown-out .codex-tmp/next-major-readiness/current.md` to snapshot the current next-major readiness state for handoff/review without changing evidence status.
7. Use `npm run next:operator -- --refresh --out .codex-tmp/next-major-operator/current.json --markdown-out .codex-tmp/next-major-operator/current.md` to produce the single operator packet for the next human/device execution pass.
