...
3. Generate the non-claiming platform execution handoff with `npm run platform:qa-handoff -- --out .codex-tmp/platform-qa-handoff/current.json --markdown-out .codex-tmp/platform-qa-handoff/current.md`, then fill and validate native Mac manual QA, Windows static/manual QA, and HarmonyOS device/toolchain QA receipts from real runs. The handoff writes real-run receipt commands to `.codex-tmp/*/real-run-receipt.json`, which the KO gate now reads by default when present.
4. Run the top-level gate with `npm run ko:validate -- --external <ko-evidence-review.json> --out .codex-tmp/ko-evidence/final.json` only after the external artifact and all three platform receipts pass. If a custom receipt path is used, pass `--mac-manual`, `--windows-static`, and `--harmony-device` explicitly.
5. Use `npm run next:readiness -- --refresh --out .codex-tmp/next-major-readiness/current.json --markdown-out .codex-tmp/next-major-readiness/current.md` to snapshot the current next-major readiness state for handoff/review without changing evidence status.
6. Use `npm run next:operator -- --refresh --out .codex-tmp/next-major-operator/current.json --markdown-out .codex-tmp/next-major-operator/current.md` to produce the single operator packet for the next human/device execution pass.
