...
For a full local refresh before a handoff or final-gate attempt, run `npm run next:local-evidence`, then `npm run next:local-evidence:check`. This regenerates the current non-claiming KO status, source approval request, readiness packet, platform handoff, operator packet, and local-evidence snapshot in the safe order. It still does not grant current-turn source approval, run approved-source browser evidence, perform privacy review, run Mac/Windows/HarmonyOS real QA, build, package, deploy, check Mew-Test/main site, run remote acceptance, or authorize release.
...
For the single operator view of all remaining next-major gates, run `npm run next:operator -- --refresh --out .codex-tmp/next-major-operator/current.json --markdown-out .codex-tmp/next-major-operator/current.md`. This packet is `NEXT_MAJOR_OPERATOR_PACKET_ONLY`; it consolidates readiness, source approval request, and platform QA handoff data, but it does not grant approval, run source capture, perform privacy review, run platform QA, build, deploy, or satisfy KO evidence. The `ko:next` summary must show `Current operator packet freshness: CURRENT_CLEAN_OPERATOR_PACKET` before the packet is used as the current critical path. If it shows `STALE_OR_DIRTY_OPERATOR_PACKET` because the worktree is dirty, resolve those changes under current-turn authorization before regenerating; do not discard changes unless explicitly asked. If the worktree is clean but the packet is stale, refresh the non-claiming inputs and then regenerate the operator packet. If a source approval request is based on a stale or dirty public dry-run, the operator packet marks it for fresh dry-run / approval-request regeneration before asking for current-turn approval.

The finalizer is only the post-evidence consolidation command. It now preflights the local-evidence snapshot before the final KO chain, but `npm run next:finalize -- --external <ko-evidence-review.json>` should still be run only after the privacy-reviewed approved-source artifact exists and all three real platform receipts validate as claimable.
