...
- Today tab now has a Learning Flow panel above the denser sections. It keeps `Capture on Mac` and `Close the loop` as the daily route, embeds the returning-user Next Move, and leaves the device return path as a lower-frequency manual drawer.
