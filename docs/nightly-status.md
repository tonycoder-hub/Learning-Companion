...
- HarmonyOS handoff now includes a DevEco/ArkTS scaffold directory plus a pure import/patch boundary module covered by `smoke:harmony`; it is structure/schema-parity checked, not SDK-compiled. The scaffold next-action contract matches the web Focus Brief shape, including `open_source`, detail, reason, open-question counts, and optional `secondaryAction` for phone-side resume decisions.
