...
- Service worker cache key is now `learning-companion-static-v4`; the activate handler only evicts older `learning-companion-static-*` caches. This was bumped after stale cached UI showed older Return JSON copy while source files had already moved to Return File wording.
