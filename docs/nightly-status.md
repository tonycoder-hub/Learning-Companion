...
- Browser smoke routes its own download artifacts to a private `$TMPDIR/lc-browser-smoke-*/downloads` path only through an explicit smoke harness flag, cleans the smoke root after Chrome exits, and includes a negative controlled-browser case so ordinary automation does not keep filling Downloads with JSON.
