...
- `Paste Return File` now previews a copied inbox/review return JSON before applying it: Device Flow shows parsed deltas, Apply, and Discard; workspace content and imported patch ids stay unchanged until Apply. The preview is in-memory only, refreshes instead of applying if the Mac workspace changed after preview, and the panel says file-picker `Import Return Files` still applies selected files immediately.
