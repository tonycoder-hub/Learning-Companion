...
- Returned phone/Windows answer imports now carry answer-specific next steps into the `Returned from phone/Windows` card: resolved questions with refreshable cards get a `Refresh cards` action into Closed Today.
...
- Older return-file compatibility imports show `legacy mirror check` when the app had to compare the old full-workspace fingerprint instead of the newer return-base projection, and the receipt/Device Flow now tells the user to re-export the mirror before the next phone or Windows study pass.
...
- Static return pages show stable suggested return-file names for Copy/Save, so a phone or Windows user has one name to move back to the Mac.
- Static return pages include a `Manual Copy` fallback for locked-down browsers where clipboard or picker permissions are unavailable; it only selects the visible return-file JSON and does not create a download.
- Device Flow has local handoff status for the manual mirror loop: latest Mirror export, return-base freshness, derived waiting-for-return state, and latest return-file import counts. Its collapsed summary now leads with the next step (`Next: export mirror`, `Mirror ready`, `Mac changed`, or `Return imported`) before the counts, so the drawer reads as a route instead of a ledger. The state is kept in local UI preferences and is not serialized into workspace or mirror exports.
- Service worker cache key is now `learning-companion-static-v4`; the activate handler only evicts older `learning-companion-static-*` caches. This was bumped after stale cached UI showed older Return JSON copy while source files had already moved to Return File wording. The web shell now shows an `App update ready` reload notice when a new service worker is waiting or takes control after an already-controlled page. The worker still uses its existing `skipWaiting()` / `clients.claim()` behavior, while the app keeps page reload manual and does not unregister workers or clear browser state. The app script installs compatibility-only shell nodes so stale HTML plus newer JS does not crash on recently added non-data UI, and browser smoke exercises that path with a virtual stale shell.
...
Tonight's no-delete validation used `LC_KEEP_SMOKE_ARTIFACTS=1 npm run smoke` and `LC_KEEP_SMOKE_ARTIFACTS=1 npm run smoke:browser`; both passed while leaving their `.codex-tmp/` run artifacts for later cleanup.
