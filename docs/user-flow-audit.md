...
- Sidecar layout now also hides the redundant Focus Brief card only when the rail already says the next step is plain capture and there is no draft, warning, review, or question pressure. Drafts and loop work still surface Focus Brief, but the high-frequency read-and-capture screen no longer repeats `Capture` three times.
