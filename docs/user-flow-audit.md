...
- Mira correctly flagged full-workspace stale-base fingerprints as potentially noisy. I accepted clearer `mirror base changed` copy plus match/null smoke coverage, but deferred a narrower return-base fingerprint projection because the current static pages already generate `source.workspaceFingerprint` from the full sanitized workspace and a projection change needs a contract update, not a rushed receipt-only patch.
...
- Return Files receipts now surface stale mirror bases with a `mirror base changed` marker when the return JSON was created from an older mirror fingerprint. This keeps manual phone/Windows transfer honest without hard-rejecting append-only captures or review events that still pass their own duplicate/version checks.
