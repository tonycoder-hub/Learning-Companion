...
- Today and mirror exports include the active session's Focus Brief so cross-device handoff starts at "resume here," including the latest captured timestamp when the source supports jumps. On Mac, Quick Capture exposes the same source action visibly as `Resume @ time`, `Open source`, or `Set source` so source setup is not hidden behind a disabled button.
- Mirror bundle includes static `index.html` as a folder home page for Today, Review, Restore, and sessions, with a Manual Return checklist that says to read Today, work in Review/Inbox, then bring Return JSON back to Mac because the mirror is not live sync. New static mirror pages also show a `Return-ready mirror` badge to clarify that Review/Inbox return JSON includes the Mac return-base check.
