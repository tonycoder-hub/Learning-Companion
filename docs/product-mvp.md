...
- Quick Capture includes local starter buttons for `Question`, `Answer`, and `Takeaway`, plus app-focused `Cmd/Ctrl+Shift+1/2/3` shortcuts when focus is not inside an editable field. They only seed the Thought draft and keep focus in the capture pane; saving, card creation, and question closure still use the existing Capture/Card/model rules.
...
- Linked-source question receipts now form a full local study loop: save the question, resume the source for evidence, start a linked answer draft, save the answer to close the question, refresh any existing question review card with that answer evidence, then resume the source again for the next point. Each step is guarded against stale questions, missing sources, or removed review cards and remains Mac-local UI state.
- Quick capture for quote + note + tags, with per-session draft recovery and a Today resume entry while switching learning contexts. Today draft cards now show when a local draft began on a different source than the current topic source, so the user can spot drift before resuming.
- Quote-only saves are treated as local highlights in the activity strip, with next-step copy that says the source page is unchanged and makes `Add thought` the immediate activity action. That action is quote-only-gated: once a capture already has a thought, the same activity no longer reopens annotation as if it were still an empty highlight. For a fresh highlight, it opens the inline annotation form in Captures, or the Recent Stack form while sidecar layout is active, without forcing an inspector detour. Recent Stack and Captures expose the same inline `Add thought` path, annotating the saved highlight instead of creating a duplicate capture. In this slice annotation is one-shot: once a thought exists, the capture becomes an ordinary quote+thought capture, keeps the source quote, bumps `updatedAt`, and does not touch the source page. If that capture was already inserted into Notes through the generated capture block, annotation refreshes that existing block; it does not auto-insert unrelated highlights into Notes. This borrows the highlight-plus-annotation pattern from reader/clipper tools without adding browser automation or sync claims.
...
- Sidecar focus rail that appears only after the side panels are hidden, keeping `Source`, `Capture`, and `Loop` actions reachable while the app sits beside a browser video or document; sidecar layout hides the metric row and collapses redundant plain capture/source Focus Brief states so the rail replaces dashboard density instead of stacking on top of it. Capture-focused activity actions now say `Capture` or `Resume`, and quote-only highlight receipts say `Add thought`; those stay inside sidecar instead of forcing an `Exit + Details` detour. The Quick Capture destination chip wraps at narrow widths so the user can still see the target topic before typing.
...
- Today keeps Learning Flow as the first action surface, then shows a compact section map so the heavier study cockpit can jump directly to due cards, questions, answers, closed items, drafts, and recent captures without making counters the first thing to inspect.
- The installable web shell keeps service-worker updates visible with an `App update ready` notice and a manual `Reload` action, so stale cached UI does not quietly undercut Learning Flow changes.
- Today includes a compact Learning Flow panel: the daily Mac track is `Read source` -> `Capture on Mac` -> `Close the loop`, while the lower-frequency device return path lives in a `Manual transfer` Device Flow drawer whose collapsed label names the next step (`Next: export mirror`, `Mirror ready`, `Mac changed`, or `Return imported`) before nonzero import counts. Mirror-content freshness uses the full workspace fingerprint so ordinary Mac captures after export make the phone/Windows mirror visibly stale; the stale drawer also shows a compact `Mirror contents changed` detail for authored deltas such as new captures, review cards, or open questions, while avoiding time-derived due-card precision. Return-file imports still validate the narrower return-base fingerprint for merge safety. After a successful return import, Device Flow records the post-import workspace fingerprint and shows `Return imported` until another local Mac change makes a fresh mirror necessary; older return receipts without that post-import fingerprint use an explicit `legacy check` label instead of silently claiming the newer state. The primary Device Flow action follows that state, with a short next-step hint beside the controls: import while a current mirror is waiting for return files, re-export when Mac content changed, and export a fresh mirror after returned work lands. `Read source` is compact for a linked source, prominent only for a resumable timestamp or missing source, and does not claim the app verified playback or page state.
- The same `Read source` / `Capture` / `Close loop` route is compressed into a sidecar-only rail after the user hides side panels, so the focused Mac layout still has a natural next action without reopening the dashboard. The clear-loop state says `Today` before exiting sidecar layout.
- The first-run Start Here actions and returning-user Next Move are embedded inside Learning Flow, so the entry point is one route instead of separate onboarding, next-action, and handoff cards. Start Here uses the same `Read source` state as the main route: open/resume an existing source when available, or set the source before capture when the topic is empty.
...
- Open questions in Today can seed an Answer draft in the source topic so the question becomes a focused capture rather than a context switch; if that topic already has an unrelated text draft, or a video timestamp-only draft that anchors the current moment, the app preserves it and asks the user to finish or clear it before answering, while clicking Answer on the same linked draft resumes it without overwriting the partial answer.
...
- Review grading keeps the user moving: when more due cards remain, the Activity strip points to `Next card` instead of the card just graded; when the queue clears, it returns to Quick Capture and can offer `Resume source` for source-backed cards.
...
- Generated `TODAY.md`, the Morning Review Pack, per-session Markdown, synthesis drafts, and mirror `README.md` now carry English source-of-truth labels plus adjacent Chinese aliases/notes for the generated shell, so Feishu/Windows/mobile handoff documents are not English-only. User-authored text, source URLs, mirror file paths, schema names, role strings, and byte counts remain unchanged. QA receipts and platform-specific manual QA templates remain separate bilingual surfaces.
- Import/return receipt banners, Device Flow latest-import summaries, and Returned Work nudges now use English/Chinese display formatters while preserving receipt schema keys, stable action `kind` values, file names, patch IDs, and user-authored content. Browser-executed language switching proof and QA receipt templates remain separate validation surfaces.
...
- Mirror bundle includes static `index.html` as a folder home page for Today, Review, Restore, and sessions, with a Manual Return checklist that says to read Today, work in Review/Inbox, then bring return files back to Mac because the mirror is not live sync. The home page now chooses one `Next from this export` action: review due cards, answer the next open question through prefilled Inbox, read or resume the safe source link when the queue is clear, or capture on the device when there is no source. Due review wins because review debt is the most time-sensitive exported work, but mixed states include a clickable secondary static deep-link into `inbox.html` so open questions are not reduced to passive copy. Source-only states keep a tappable `Then capture in Inbox.` secondary action, external source links open separately so the mirror page remains available for the return-file step, and the source action tells the user to come back to the mirror tab for return JSON. The static Inbox page now shows the selected topic source under the Topic selector, explains that new captures use it unless Source/URL is filled below, and switches to override copy as soon as the user types a Source or URL. Static pages include a `<noscript>` failure path explaining that the mirror remains readable but Review/Inbox return-file creation needs JavaScript. New static mirror pages also show a `Return-ready mirror` badge to clarify that Review/Inbox return files include the Mac return-base check via `source.returnBaseFingerprint`.
- Static Review and Inbox pages now keep that mixed-lane context after the user saves a return file: Review can point to the prefilled Inbox answer link when the same mirror still has open questions, and Inbox can point back to Review when due cards remain. These are post-save relative links only; the return JSON schema is unchanged and the copy still says manual return files, not live sync.
- The post-save static return panel leads with the phone-safe transport path: use Copy or Manual Copy, paste the return JSON into a note/email/message, send it to Mac, then import or paste from Today > Return Files. AirDrop, USB, file share, and manual Feishu Drive remain optional carriers after a JSON file exists; the static page does not claim a share sheet or live sync.
- Mirror home open-question previews link into `inbox.html` with a prefilled Answer draft and a visible answer-context banner. In that answer-link mode the carried Mac question is labeled `Question from Mac` and is read-only, while the writable field is labeled `Answer to return`; after adding the answer to the local return draft, the form returns to ordinary `Quote` / `Thought` capture mode. This lets phone/Windows/Feishu folder review return an append-only answer patch without mistaking the static mirror for live sync or silently editing the Mac question.
...
- Return Files can import multiple inbox/review return files in one picker action and reports per-file batch counts for inbox additions, review grades, duplicates, and failures. Batch import applies inbox returns before review returns and continues past wrong-type files with per-file errors.
- Return Files receipts warn with `mirror base changed` when a phone/Windows return file came from an older return-base fingerprint, name the affected files in batch imports, and still allow append-only captures or version-matched review events to merge safely.
- Older return files that only carry a full workspace fingerprint are labeled `legacy mirror check` in receipts, so the user can tell the app used the compatibility path rather than the newer return-base projection.
- Import failures leave a visible issue receipt for bad mirror payloads, malformed JSON, and oversized patches.
- Today tab surfaces Return Files counts, latest receipt, and the manual device-labeled path inside Device Flow: export a mirror on Mac, transfer it yourself through USB, AirDrop, email, file share, or manual Feishu Drive upload, use `inbox.html` or `review.html` on phone/Windows, then import the returned JSON on Mac. The export action opens the Export tab directly at the Mirror Folder controls, and mirror saves leave a handoff receipt with both full mirror-content freshness and return-base safety state.
...
- When a return-file import adds new captures or review updates, Learning Flow shows a dismissible `Returned from phone/Windows` nudge that jumps to returned captures or import details. Duplicate-only imports stay in the receipt without adding a new primary-card distraction.
...
Post-save Quick Capture receipts are derived from committed captures, not uncommitted starter drafts. The starter shortcuts are app-focused draft helpers only and do not run inside editable fields, commit captures, or create review cards. A `Question` receipt requires either a question mark in prose or an explicit `Question:` / `Q:` prefix with a non-empty body, then routes to the Today Open Questions ledger. Answer receipts distinguish linked answers that close a question from standalone answer notes that only appear in Answers Today. Takeaway receipts stay in the Mac capture surface. These receipts are intentionally not exported as state to Harmony/Windows static mirrors, Return JSON, workspace JSON, or Feishu mirrors.
