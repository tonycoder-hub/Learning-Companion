...
- Mirror bundle includes static `index.html` as a folder home page for Today, Review, Restore, and sessions, with a Manual Return checklist that says to read Today, work in Review/Inbox, then bring return files back to Mac because the mirror is not live sync. The home page now chooses one `Next from this export` action: review due cards, answer the next open question through prefilled Inbox, or capture on the device when the queue is clear, with the export timestamp shown beside the action. Due review wins because review debt is the most time-sensitive exported work, but mixed states include a clickable secondary static deep-link into `inbox.html` so open questions are not reduced to passive copy. New static mirror pages also show a `Return-ready mirror` badge to clarify that Review/Inbox return files include the Mac return-base check via `source.returnBaseFingerprint`.
