...
- Return Files receipts warn with `mirror base changed` when a phone/Windows return JSON came from an older mirror base, while still allowing append-only captures or version-matched review events to merge safely.
