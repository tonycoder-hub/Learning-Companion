...
- Today includes a compact Learning Flow panel: the daily Mac track is `Read source` -> `Capture on Mac` -> `Close the loop`, while the lower-frequency device return path lives in a `Manual transfer` Device Flow drawer whose collapsed label names the next step (`Next: export mirror`, `Mirror ready`, `Mac changed`, or `Return imported`) before nonzero import counts. Mirror-content freshness uses the full workspace fingerprint so ordinary Mac captures after export make the phone/Windows mirror visibly stale; return-file imports still validate the narrower return-base fingerprint for merge safety. `Read source` is compact for a linked source, prominent only for a resumable timestamp or missing source, and does not claim the app verified playback or page state.
